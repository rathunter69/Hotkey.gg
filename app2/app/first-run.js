...
export const PLACEMENT_TASKS = [
  { id: 'move', text: 'Make C4 (Tuesday, Units) the active cell', keys: 'arrow keys, or Ctrl+Arrow to jump', skips: ['jump-dont-scroll'],
    check: (s, ses) => !ses.editing && !s.sel && s.selectionText() === 'C4' },
  { id: 'select', text: 'Select the range B2:B6', keys: 'Shift+Arrow from B2', skips: ['select-like-you-mean-it'],
    check: (s, ses) => !ses.editing && !!s.sel && s.selectionText() === 'B2:B6' },
  { id: 'type-bold', text: 'Type Total in A8, then make A8 bold', keys: 'type, Enter, then Ctrl+B or Alt H B', skips: ['ribbon-by-keyboard'],
