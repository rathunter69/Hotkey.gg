...
const allTotals = sh => TOTAL_ROWS.every(r => totalFormula(sh, r));
...
export default {
  ...
  goals: [
    { id: 'ref-fix', teach: 'Each error code is a message: #REF! a deleted reference, #NAME? a name Excel does not know, #VALUE! text where a number belongs, #N/A a lookup that found nothing.', text: 'On Costs, Mueller’s total E5 says #REF!: press F2, Backspace the dead #REF! tail, press F2 again and point at D5 to finish =B5+C5+D5.', keys: 'Ctrl+PgDn ×3 Ctrl+↓ Ctrl+→ ↓ ×2 F2 ⌫ ×5 F2 ← ↵', requires: ['formula-errors', 'edit-mode-f2', 'backspace', 'pointing', 'sheet-tabs', 'ctrl-arrow', 'arrow-keys'], convention: 'F5',
      check: (s, ses) => { const sh = costs(ses); return !!sh && totalFormula(sh, 5) && settled(ses); } },
    { id: 'name-fix', text: 'E6 says #NAME?: SUMM is not a function, so press F2, Home, → ×3 to sit after SUM, Delete the extra M, and Enter.', keys: 'F2 Home → ×3 Delete ↵', requires: ['formula-errors', 'edit-mode-f2', 'edit-caret'],
      check: (s, ses) => { const sh = costs(ses); return !!sh && totalFormula(sh, 5) && totalReads(sh, 6) && settled(ses); } },
    { id: 'value-trail', text: 'E7 says #VALUE!: Ctrl+[ jumps to the first cell it reads, B7, and along that row D7 holds the word tbc: fill 260 down from D6.', keys: 'Ctrl+[ → ×2 ↑ Shift+↓ Ctrl+D', requires: ['formula-errors', 'fill-down-right', 'shift-arrow', 'arrow-keys'], convention: 'F3',
    ...
    { id: 'formats-back', text: 'Back on Costs, E5:E8 are formulas, not inputs, and must read black like E4: copy E4 and paste its format only onto them with Ctrl+Alt+V, T.', keys: 'Ctrl+PgDn ×3 ↑ ×2 Ctrl+→ Ctrl+↑ ↓ Ctrl+C ↓ Shift+↓ ×3 Ctrl+Alt+V T ↵', requires: ['input-colour-convention', 'copy-cut-paste', 'paste-special', 'sheet-tabs', 'ctrl-arrow', 'shift-arrow', 'arrow-keys'], convention: 'B1',
  ...
  endState: [
    { text: 'E5:E8 are the block’s one formula, lease + maintenance + network, and read numbers', check: (s, ses) => { const sh = costs(ses); return !!sh && allTotals(sh); } },
  ...
  closing: [
    'Five error codes, five messages read and answered at the source: a deleted reference, a misspelled name, a word where a figure belongs, a lookup that found nothing, a division by an empty cell (F5), and Ctrl+[ followed each trail to the cell the formula reads, across sheets too (F3).',
