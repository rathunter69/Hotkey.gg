...
export default {
  ...
  goals: [
    { id: 'costs-comma', text: 'On Costs, select the three cost columns B4:D8 and give them thousands separators with no decimals in one action, Ctrl+1 then N.', keys: 'Ctrl+PgDn ×3 Ctrl+↓ ↓ → Ctrl+Shift+↓ Shift+→ ×2 then Ctrl+1 N', requires: ['number-formats', 'format-cells-dialog', 'sheet-tabs', 'ctrl-arrow', 'ctrl-shift-arrow', 'shift-arrow'], convention: 'D2',
      check: (s, ses) => { const sh = costs(ses); return !!sh && fmtIs(sh, COST_COLS, 'comma', 0) && settled(ses); } },
    { id: 'f4-repeat', teach: 'F4 outside a formula repeats your last action, a format, a border, a width, an insert, on whatever is selected now: format once, F4 everywhere.', text: 'The Total column needs the same format: select E4:E8 and press F4 once; the whole Format Cells action you just did lands on all five cells.', keys: 'Ctrl+→ Ctrl+Shift+↓ F4', requires: ['f4-repeat', 'ctrl-arrow', 'ctrl-shift-arrow'],
    ...
    { id: 'headers-right', text: 'The four headers B3:E3 sit over numbers, so right-align them with Alt H A R; Site in A3 sits over names and stays as it is.', keys: '↑ Ctrl+Shift+← Shift+→ Alt H A R', requires: ['align-command', 'keytips', 'ctrl-shift-arrow', 'shift-arrow'], convention: 'D6',
