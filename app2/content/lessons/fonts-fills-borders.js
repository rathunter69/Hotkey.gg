...
export default {
  ...
  brief: 'The Report’s figures now read right, but nothing on the page tells the eye where to look: the title, the header row and the total row all sit in plain text. Bold the title and size it up, bold the header and total rows, give the total row a top border, and tint the input block on Inputs. The key is `Alt H B`.',
  goals: [
    { id: 'title-bold', teach: 'Ctrl+B makes the selection bold, Ctrl+I italic and Ctrl+U underlined; press it on a mixed selection and every cell goes on.', text: 'The title in A1 is plain text: make it bold with Ctrl+B so the page has a head.', keys: 'Ctrl+B', requires: ['bold-italic-underline'],
    ...
    { id: 'header-row', text: 'Site and Week in A4:B4 were never bold: select the whole header row 4 with Shift+Space and press Ctrl+B once for every header.', keys: '↓ ×3 Shift+Space Ctrl+B', requires: ['bold-italic-underline', 'row-col-select'],
