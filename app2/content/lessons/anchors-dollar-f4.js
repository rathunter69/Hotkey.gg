...
const gridCell = (sh, col, r) => anchored(sh.formula(col + r)) === `=$C${r}*${col}$${PRICE_ROW}` && near(sh.value(col + r), sh.value('C' + r) * sh.value(col + PRICE_ROW));
...
export default {
  ...
  brief: 'The associate, the colleague who checks your page, wants each site’s weekly energy cost at three wholesale prices: a grid beside the report, sites down, prices across. One formula covers all eighteen cells if its two references are anchored the right way, and F4 sets the anchors for you so you never type a $. The key is `F4`.',
  goals: [
    ...
    { id: 'fill-down', text: 'Now the five sites below: extend the selection to J5:L10 and fill down with Ctrl+D, and J$4 stays on the price row while the kWh moves.', keys: 'Shift+↓ ×5 Ctrl+D', requires: ['fill-down-right', 'relative-absolute', 'shift-arrow'],
      check: (s, ses) => { const sh = report(ses); return !!sh && gridLive(sh) && settled(ses); } },
    { id: 'read-back', text: 'Read one from the middle of the grid before you trust it: on K7 press F2, see $C7 and K$4 lit in color, then Esc.', keys: '→ ↓ ×2 F2 Esc', requires: ['edit-mode-f2', 'escape-cancels', 'arrow-keys'],
      check: (s, ses) => { const sh = report(ses); return !!sh && gridLive(sh) && windowKeys(ses).includes('F2') && settled(ses); } },
    { id: 'clear-stub', text: 'The old sensitivity stub under the daily table is now covered by the grid: select A23:F24 and clear it with Delete.', keys: 'Ctrl+← Ctrl+↓ ×4 ↓ Ctrl+Shift+→ Shift+↑ Delete', requires: ['delete-clears', 'ctrl-arrow', 'ctrl-shift-arrow', 'shift-arrow', 'arrow-keys'],
