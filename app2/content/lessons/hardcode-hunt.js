...
/** Go To Special was opened this goal (the Home › Find & Select › Go To Special walk, Alt H F D S). */
const goToSpecialUsed = ses => { const w = windowKeys(ses); return w.some((k, i) => k === 'Alt' && w[i + 1] === 'H' && w[i + 2] === 'F' && w[i + 3] === 'D' && w[i + 4] === 'S'); };
...
export default {
  ...
  brief: 'The associate’s marked-up copy of the Report came back with eight faults from the day-one list, and the page goes to the buyer tonight. The audit pass finds them the way a reviewer would, with Go To Special, show formulas and your eyes on the page, and you fix every one without changing anything else. The key is Ctrl with the backtick key, `Ctrl+`.',
  goals: [
    { id: 'find-constants', teach: 'The audit pass is three looks: Go To Special for typed numbers among links, show formulas for a number in a formula row, the page for the rest.', text: 'Typed numbers hide among the links: select the figure block C5:E10, run Go To Special for Constants, and color Cedar Park’s emailed figures blue.', keys: 'Ctrl+↓ ↓ → ×2 Ctrl+Shift+↓ Shift+↑ Shift+→ ×2 then Alt H F D S O ↵ then Alt H F C → ×4 ↵', requires: ['audit-pass', 'go-to-special', 'font-color', 'input-colour-convention', 'ctrl-arrow', 'ctrl-shift-arrow', 'shift-arrow', 'keytips', 'arrow-keys'], convention: 'F3',
      check: (s, ses) => { const rep = report(ses); return !!rep && cedarTypedBlue(rep) && goToSpecialUsed(ses) && settled(ses); } },
    { id: 'f4-energy', text: 'Cedar Park’s energy cost E9 is built on the emailed figure, so it reads as an input too: move to E9 and press F4.', keys: '→ ×2 F4', requires: ['f4-repeat', 'input-colour-convention', 'arrow-keys'], convention: 'B1',
      check: (s, ses) => { const rep = report(ses); return !!rep && cedarTypedBlue(rep) && blue(rep, 'E9') && windowKeys(ses).includes('F4') && settled(ses); } },
    { id: 'literal-in-formula', text: 'Press Ctrl+` to show formulas: Mueller’s average price G6 divides by a typed 6850, so rewrite it as =D6/C6 by pointing.', keys: 'Ctrl+` Ctrl+→ ← Ctrl+↑ ↓ ×2 "=" ← ×3 "/" Ctrl+← → ×2 ↵', requires: ['show-formulas', 'pointing', 'formula-operators', 'ctrl-arrow', 'arrow-keys'],
    ...
    { id: 'units-line', text: 'The units line is gone, so a reader cannot tell dollars from thousands: in A2 press Ctrl+I, then type USD unless stated.', keys: 'Ctrl+← ↓ Ctrl+I "USD unless stated" ↵', requires: ['bold-italic-underline', 'type-to-enter', 'ctrl-arrow', 'arrow-keys'],
  ...
  endState: [
    { text: 'Cedar Park’s emailed row C9:E9 reads blue; G6 and E18 are formulas again', check: (s, ses) => { const rep = report(ses); return !!rep && cedarTypedBlue(rep) && blue(rep, 'E9') && g6Fixed(rep) && e18Linked(rep); } },
  ...
  closing: [
    'Eight faults found in a colleague’s file in a few minutes, the way a reviewer finds them: Go To Special for the typed numbers, show formulas for the literal and the retyped Thursday, the page for the rest (F3).',
