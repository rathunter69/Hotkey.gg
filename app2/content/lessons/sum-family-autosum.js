...
/** AutoSum was pressed this goal: the chord Alt+= (logged 'Alt' then '='), or the Ribbon's Home › AutoSum walk Alt H U S. */
const autoSumUsed = ses => { const w = windowKeys(ses); return w.some((k, i) => k === 'Alt' && (w[i + 1] === '=' || (w[i + 1] === 'H' && w[i + 2] === 'U' && w[i + 3] === 'S'))); };
...
export default {
  ...
  brief: 'The total row is missing its Gross profit SUM, the total’s average price and margin are still blank, and the associate wants a week summary under the report: the average, best and lowest site, and a count of each. AutoSum over the whole block writes every column’s SUM in one press, and the SUM family covers the rest. The key is `Alt+=`.',
  goals: [
    ...
    { id: 'total-avg-price', text: 'The total row’s average price is the total revenue over the total kWh: in G11, point =D11/C11 and commit with Tab.', keys: 'Ctrl+↓ Ctrl+→ → "=" Ctrl+← ← ×2 "/" Ctrl+← ×2 Tab', requires: ['pointing', 'formula-basics', 'formula-operators', 'ctrl-arrow', 'tab-commits'],
      check: (s, ses) => { const rep = report(ses); return !!rep && ratio(rep, 'G' + T, 'D' + T, 'C' + T) && !ses.editing; } },
    { id: 'total-margin', text: 'The total row’s margin is the total gross profit over the total revenue: in H11, point =F11/D11 and press Enter.', keys: '"=" Ctrl+← ← "/" Ctrl+← ×2 → ↵', requires: ['pointing', 'formula-basics', 'formula-operators', 'ctrl-arrow'],
      check: (s, ses) => { const rep = report(ses); return !!rep && ratio(rep, 'H' + T, 'F' + T, 'D' + T) && !ses.editing; } },
    { id: 'summary-labels', text: `Head a Week summary block under the report: Week summary in A26, bold, then the five line labels A27:A31 as one Enter run.`, keys: `Ctrl+← Ctrl+↓ ×4 ↓ ×2 Ctrl+B "Week summary" ↵ ${LABELS.map(l => `"${l}" ↵`).join(' ')}`, requires: ['type-to-enter', 'bold-italic-underline', 'ctrl-arrow'],
    ...
    { id: 'summary-format', text: 'The three kWh lines B27:B29 read as figures: select them and give them thousands separators with no decimals, Ctrl+Shift+1 then Alt H 9 twice.', keys: '↑ ×3 Ctrl+Shift+↓ Ctrl+Shift+1 then Alt H 9 Alt H 9', requires: ['number-formats', 'ctrl-shift-arrow'],
  ...
  closing: [
    'One press of Alt+= over the whole block wrote every column’s SUM into the total row at once, Gross profit included: AutoSum the block plus its edge, never one column at a time (E5).',
