...
for (const l of LESSONS) for (const c of l.concepts || []) if (!(c in LESSON_BY_CONCEPT)) LESSON_BY_CONCEPT[c] = l.id;

/** The id of the first lesson that teaches a concept, or null. */
...
const NATIVE = [
  ['Navigation', [
    ...
    R('ctrl-page-up-down', 'Ctrl+PageUp/PageDown', 'Previous / next worksheet', 'Move to the previous or next worksheet.', { note: 'Compact keyboards: Ctrl+Fn+↑ / Ctrl+Fn+↓' }),
  ]],
  ['Selection', [
  ...
  ['Formatting', [
    ...
    R('alt-h-f-c', 'Alt H F C', 'Font color', 'Font color.'),
  ]],
  ['Borders', [
  ...
  ['Formulas and fill', [
    ...
    R('f9', 'F9', 'Recalculate', 'Recalculate all open workbooks.'),
