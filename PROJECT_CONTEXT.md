...
- **PENDING — live smoke test (BLOCKED ON WOLF, see P0 above)**: r131 ran the
  first live probe and found the migrations never deployed — the RPC smoke
  matrix is now committed at dev/smoke-live.mjs and runs the moment the deploy
  workflow goes green. Seeded desk codes are in migration 20260712400000 —
  Wolf distributes them only after smoke is green. Claim-vs-domain-join
  deadlock pre-fixed in 20260712700000.
