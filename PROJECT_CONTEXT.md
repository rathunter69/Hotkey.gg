...
5. NO-RECALC RULE (engine finding, r94): the engine does NOT recompute
   dependents on edit. Planted defects must sit on cells NOTHING ELSE reads
   (or checks must judge against DISPLAYED upstream values). The first 4am-
   pass design violated this and was unsolvable in one column — harness
   caught it pre-ship. A real dependency-recalc pass is a candidate engine
   upgrade (same turn as IF/comparators).
6. STICKY-IF/scenario drills GATED on evaluator IF() + comparators (engine
   has SUM/AVERAGE/MIN/MAX/ABS/SUMIF/INDEX/MATCH — verified r94).
