...
| A | Site shell and onboarding | Full site, landing, first run, catalog, lesson workspace, ribbon, reference, Welcome + How-Excel-works, effects | DONE pending Wolf's final sign-off EXPERIENCE PASSES A-C DONE 2026-09-25: landing, first run, Home, the Learn data room, the floating task card, reward moments, loading and error states, density, the smoke journey (first run to the Daily, failure paths, 64 s). WAITING ON WOLF: four calls (daily-user placement, the paid-sample line, workspace width on big monitors, Ctrl+Shift+J/K) |
...
| E | Paid tier | Chapters 2-3 lessons + samples; entitlements processor-agnostic; then merchant-of-record checkout (test mode), student verification, group codes, pricing wiring | GATE LIVE 2026-09-26: the client reads its own entitlements rows (app/entitlement.js) and Chapter 2 opens only to an entitled account — admin/redeem grants (B) are the way in; no checkout yet. Exit check: test-mode purchase, cancel, expiry, refund, group grant pass end to end |
