...
- Headline: "Excel isn't learned. It's practiced." (Wolf's pick, 2026-09-23; the page title matches it). Body: whether you're a beginner or a finance professional, learn Excel and practise through guided lessons and real-world drills, not by watching a video or sitting in a classroom. Hero visual: the old preview card upgraded into a live, self-playing mini lesson (keycaps press, cells format, a goal ticks off) that hands control to the visitor the moment they type. Sections: how it works (read, do it guided, beat the challenge, race the clock), what you'll learn (the 6 chapters), free vs paid, for teams and schools, footer.
...
## 4. Lesson workspace
- The task card floats over the sheet by default (Wolf, experience passes B and C): it sits beside the goal's target and never covers the active cell, the selection or what the goal asks the learner to read; it folds to a pill while a cell is being edited (150 ms delay) and opens again on commit or Esc. Ctrl+Shift+J moves it to the next side, Ctrl+Shift+K hides or shows it; both are listed in the strip's More menu. Docked right (the old split pane: sheet and ribbon on the LEFT, panel on the RIGHT, divider draggable within limits) and docked left are settings in that menu.
...
- Panel tabs: Lesson | Help | Shortcuts used. Phases: Brief -> Guided -> complete; the module challenge replaces Try solo and the per-lesson timed variant.
...
- Lessons are 5-7 minutes: one job, 5-8 goals, on a module workbook; each module ends in a 2-3 minute timed challenge. Chapter 1 works one growing workbook (`voltline-weekly`); the paid chapters bring their own.
- Grid: the engine grid stays 100 × 26 for Chapter 1; Chapter 4's exports are capped at 90 rows by it. Widening the grid (rows/cols are already parameters on `Sheet`; the cost is in `sheet-view.js` virtualisation and `selectAll`'s `A1:Z100` constant) is the one engine change that would let Chapter 4 use realistic exports. Not in this phase; recorded as an open decision.
- Completion: the familiar centered overlay. Continue is primary; on the module's last lesson, Start the challenge beside it. Shows time, keystrokes, assisted/clean.
...
- Help: reading explanations is always free. Revealing steps ("show me") in a lesson gives reduced XP; the module challenge earns the rest. Any help in a timed run means no PB and no board entry.
...
- Challenge passed: the tier stamp (pass / pro / legendary) animates onto the module card, permanently shown in the catalog.
...
## 7. Curriculum
The curriculum map (accepted 2026-09-23, from `docs/proposals/CURRICULUM_MAP.proposed.md`): Project Volt, six chapters, Chapter 1 in full, Chapters 2-6 as listed. Chapter 1 is free. Content is written from zero; the old 74 drills are ideas only and are not ported. Every chapter ends with a project and an assessment (used for the Verified certificate); Chapter 1 also has a test-out. Not in scope, by design: VBA and macros, Power Query and Power Pivot, LAMBDA/LET, charts and form controls, add-ins.
...
### 1.0 Welcome (retired 2026-09-24: its sixty-second race opens 1.1.1; nothing ships under this number)
...
- About 40 achievements at launch, all meaningful: section and chapter completions, challenge streaks, first PB, par tiers, Daily participation, efficiency feats, and a few hidden playful ones. Main milestones visible; hidden ones revealed when earned.
...
- **Achievements in the MVP (~40):** four flavours, all wanted — milestones (first lesson, each section, each chapter, first challenge/timed/PB/Daily), skill feats (keyboard-only, no-help, under-par, clean challenge passes, clean audit), speed/streak (practice-day streaks, N drills in a day, beat pro/legendary, top-N on a board), and hidden/playful (cheeky secrets revealed only when earned). Pixel-art, four rarities, no XP.
