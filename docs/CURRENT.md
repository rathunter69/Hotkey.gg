...
| **Audit catalog and learner flow**, `01a0b148-2693-7901-af91-934c83cabe1d`; `codex/catalog-flow-review` from preserved 00df57a | Proposal checkpoint 08d1e46 verified. Earlier solo/guest/clock/retry/practice/sharing/Help approvals and simplicity constraint remain. Two new choices are presented and pending: Windows/Mac support promise and single-XP reward principle. | Owns only `docs/handoffs/catalog-progression.md`; await answers, then concrete first-lesson/helped-result/returning-home examples. No platform compatibility claim, Mac regression permission, reward formula, implementation or new module approved. |
...
| Catalog/flow decision review | [08d1e46](https://github.com/rathunter69/Hotkey.gg/blob/08d1e465218c0aafa2985ea19c3b7fcf3bff1576/docs/handoffs/catalog-progression.md) | Chief verified sole-doc delta from 216a2af: new platform/XP questions are pending proposals only. Prior September 19 approvals and their focused independent review remain valid. No new agents, source investigation, audit or tests in this checkpoint; PRODUCT has no new confirmed rule. |
...
The groups recorded at 216a2af are answered. Wolf then asked which other design/platform
choices would help building. **Two new questions are presented and pending** at 08d1e46:
desktop Windows primary with Mac explicitly scoped/verified versus full parity or a shared
beginner core with platform-specific advanced content; and most XP for new eligible
accomplishments with limited repeat rewards/no speed multiplier versus first-only or similar
rewards for every eligible completion. These are proposals, not selected support or XP rules.
Preserve existing Mac behavior and earned XP; helped/spreadsheet-mouse attempts still earn
no XP. No compatibility claim, exact formula, new mastery/rank system or implementation follows.

Await these two answers without duplicating the questions, then review **concrete first-lesson,
helped-result and returning-home examples** against the simplicity constraint. Other remaining
high-impact choices include representative lesson depth, certificate purpose/initial scope,
Desks prominence and exact free/paid learning outcomes. Coordinate existing owners without
starting new workstreams. The proposed default-learning/contextual Solo-Speedrun/secondary
