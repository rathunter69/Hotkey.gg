...
window.HOTKEY_DRILLS = {
  ...
  // total anywhere else.
  groups: [
    { name: 'Foundations',    keys: ['navigation', 'entrybasics', 'ribbonways', 'rowops', 'blocksel', 'pastes', 'filldr', 'editfix', 'modeltour'] },   /* r367: modeltour capstone · r424 (D17): colops RETIRED into rowops · r457: THE ONBOARDING RAMP. Three light guided tutorials (navigation · entrybasics · ribbonways) lead, then the five heavily-iterated drills follow LIGHTEST-FIRST (rowops 30 → blocksel 34 → pastes 42 → filldr 44 → editfix 52) as the graduate-into fulsome tier, then modeltour is the capstone. The navigation→filldr cliff is gone. (Curriculum-map note: the map classifies the five by TOPIC — filldr is Formulas-I content, etc. — a deliberate model-vs-packaging split; the live game keeps them in Foundations per Wolf.) */
  ...
  meta: {
    ...
    entrybasics: { name:'Enter, Edit & Fill', label:'Type, fix, fill', tab:'Entry', desc:'A half-finished run-rate table: type the two missing unit counts, fix the one rate entered with an extra zero — edit it in place, no retype — then fill the Cost column down so every line multiplies its own units by its own rate. Save when it foots' },   /* r457: Foundations 2 — the onboarding rung teaching enter / edit / fill, three steps on one small board */
    ribbonways: { name:'Ribbon & Chords', label:'Two ways to drive', tab:'Ribbon', desc:'A monthly summary that came off the model raw: comma the figures and rule the Profit line from the ribbon, then bold the header and percent the margin with chords, and mark the tax rate blue. Every command has a ribbon walk and a chord — learn to reach for both' },   /* r457: Foundations 3 — the onboarding rung teaching the ribbon-walk vs chord mental model, three steps on one summary page */
    ...
    triage:     { name:'Error triage', label:'Error triage \u2014 #REF! #DIV/0! #VALUE!', tab:'Triage', desc:'Three live errors and eight red cells \u2014 read what each one says, find the causes, rebuild what they were meant to say', errorCount:3 },   /* r439 depth pass (\u00a74.46 + \u00a72.3): errorCount puts the found-k/3 meter on the rail; desc rewritten for the real-sentinel board */
    versionup:  { name:'Roll-forward prep', label:'Replace hardcodes so it rolls forward', tab:'Rollfwd', desc:'Every rate is typed inside a formula \u2014 lift them onto the panel so the page rolls forward' },   /* r439 \u00a74.54 depth pass: desc rewritten \u2014 the old one described the retired typed-rows board */
    ...
    drill:      { name:'Hardcode', label:'Hardcode it',         tab:'Hardcode',    desc:'A snapshot still wired into the model — flatten what ships, mark it, archive it, then delete the feed and watch which column survives' },   /* r438 (§4.42): desc described the retired two-check board; rewritten to the prove-out drill */
    ...
    // Data
    sort:       { name:'Sort',     label:'Sort the table',tab:'Sort',       desc:'A pipeline log ranked by size, a seventh deal signed late — rank it, land the newcomer, rank it again, total and close the line' },   /* r436 §4.33 depth pass: the old desc described the retired 3-check board */
    ...
    cases:      { name:'Sticky switch', label:'Scenario switch with CHOOSE', tab:'Cases', desc:'One switch drives the page \u2014 a driver block repositions every assumption, and a self-referencing IF snapshots each case into the output table' },
...
window.HOTKEY_CLOCKS = {
  ...
  navigation: { pass: 225 },
  entrybasics: { pass: 150 },   /* r457: Foundations 2 onboarding tutorial — par 60 × 2.5 (§9.1 rule; keep in lockstep with HOTKEY_PARS.entrybasics) */
  ribbonways: { pass: 225 },    /* r457: Foundations 3 onboarding tutorial — par 90 × 2.5 (keep in lockstep with HOTKEY_PARS.ribbonways) */
  modeltour: { pass: 70 },
