...
window.HOTKEY_DRILLS = {
  ...
  // ---------------------------------------------------------------
  /* r457 CATALOG_V4 §2 (P1/P2) — the two VARIETY fields, on every drill. They are metadata only:
     no engine code reads them, dev/check-invariants.js C28–C31 and dev/curriculum-v4.json do.
       family   — the ONE primary skill-family id from CATALOG_V4 §3 (N/E/F/M ids). It is the
                  Excel LESSON, not the finance artifact: two drills sharing a primary family in
                  one chapter are the same drill to the hands (§1 F1/F4/F8, principle P1).
       families — the secondary ids, when §4 names more than one for the drill (ordered, optional).
       board    — what the board IS at load, from the index.html builder, one of
                  schedule (periods across, line items down) · table (labelled rows, no time
                  spine) · tape (a long record ledger/export) · form (a memo/document page) ·
                  grid (an n x m matrix whose two axes are both drivers) · two-block (two blocks,
                  one feeding the other) · list (a long one-column list worked top to bottom) ·
                  maze (a corridor board) · cover (a one-page headline/output box). P2 rations it. */
  meta: {
    // Foundations
    navigation: { family:'N1', board:'maze', families:['N2','E3'], name:'Navigate & Select', label:'Navigate and select', tab:'Nav', desc:'One long switchback corridor through solid walls: every straightaway is a single shot, every corner one turn. Collect the checkpoints, land on the block and the walls come down — a regional sales table and a memo naming four ranges to capture: one region across the quarters, one quarter down every region, the whole table, and every typed figure on it. Then copy the table, bring it home to the marked room at A1 and save' },   /* r456 (CURRICULUM_V3 §9.1): Foundations 1 — three steps on one board that grows */
    autofit:    { family:'F6', board:'table', families:['F5','M1'], name:'Autofit',  label:'Fix the squeezed columns',tab:'Autofit',  desc:'A print page nobody can read — fit the labels to their content, hold the quarter block to one width, and find out what your own totals do to a narrow column' },   /* r432 (§4.14): reworked to the single print-page board */
    rowops:     { family:'E6', board:'schedule', families:['E4'], name:'Structure', label:'Rebuild the schedule',   tab:'Structure',desc:'Full structural surgery \u2014 insert the missing line and the missing quarter, delete the squatters, and watch every new row and column arrive already dressed' },   /* r424 (D17): absorbed colops \u2014 rows AND columns in one drill */
    filldr:     { family:'E5', board:'schedule', families:['M2'], name:'Fill',     label:'Fill down, fill right',   tab:'Fill',     desc:'A quarterly operating build off the revenue feed — three cost lines across the year, EBITDA, Fiscal Year totals, and a ratio block down to EBITDA margin. One anchored formula fills most of it' },   /* r452 (audit §5.3): the old desc ("Fill down and fill right — one formula, whole block") sold a 2-beat exercise; the r427 board is 7 beats at par 44. First sentence stays inside the 158-char meta-description cut in dev/build-drill-pages.js */
    blocksel:   { family:'E3', board:'two-block', families:['F1','F5'], name:'Block Select',label:'Assemble and format the summary',tab:'Block', desc:'COPY what stays, CUT what moves — the feeds run in segment order; dress the table as you go and box it whole' },   /* r424 §4.4 round 2: margin note + money formats out; whole-table close in */
    editfix:    { family:'E1', board:'schedule', families:['E2','E3'], name:'Repair',   label:'Repair the schedule',    tab:'Repair',   desc:'A review came back on the schedule — a misspelled team, an FY formula pointing at the wrong row, a Total that never grew. Repair all three, then do what the sweep note says and watch it cost you' },   /* r431: absorbed undo — the repair pass and its safety net are one drill */
    pastes:     { family:'E4', board:'two-block', families:['M16'], name:'Paste Special',label:'Paste Special everything',tab:'Paste Sp.',desc:'The full paste-special toolkit — transpose, divide, multiply, formats, values' },
    waterfall:  { family:'M5', board:'schedule', families:['M13'], name:'Waterfall', label:'Run the paydown waterfall',  tab:'Waterfall', desc:'Foot the cash a credit page has for debt, then cascade it down two tranches by seniority' },   /* r446 §4.72 depth pass: desc rewritten to the reworked board (the old one named the answer function) */
    ...
    cascade:    { family:'M5', board:'schedule', families:['M13','M14'], name:'Full Waterfall', label:'Run the full cascade',   tab:'Cascade',   capstone:true, desc:'The Models II capstone \u2014 three facilities, four years, cash applied strictly by seniority: each layer rations against what the one above it left, every balance rolls forward, and the page closes on total debt. One clean run opens the next track leg' },
    wk13:       { family:'M11', board:'grid', families:['M1'], name:'13-Week Cash', label:'Run the 13-week', tab:'13-Week', desc:'The restructuring staple: a weekly cash roll-forward, an anchored liquidity cushion, and totals on the flow lines only' },   /* r447 §4.75: desc rewritten to the flow-block board (receipts and disbursements broken out) */
    liqbridge:  { family:'E3', board:'table', families:['M5'], name:'Liquidity Bridge', label:'Bridge the liquidity \u2014 three cases', tab:'Liq. Bridge', desc:'Cash plus revolver availability walked to ending liquidity across three cases, and the cushion against the covenant' },   /* r447 §4.74: availability now capped by the borrowing base — the old desc described the uncapped board */
    covtable:   { family:'M5', board:'table', families:['F1'], name:'Covenant Table',  label:'Run the covenant table', tab:'Covenants', desc:'Two leverage tests against a stepping maximum \u2014 headroom every quarter, a compliance flag, and the quarter that pinches' },   /* r447 §4.73: the board now runs senior secured AND total net leverage */
    txncomps:   { family:'M10', board:'tape', name:'Transaction Comps', label:'Run precedent transactions', tab:'Txn Comps', desc:'A six-deal precedent tape, the median multiple paid, and the implied equity value' },
    sourcesuses:{ family:'M14', board:'two-block', families:['M1','M4'], name:'Sources & Uses',       label:'Balance sources and uses',   tab:'S&U',       desc:'Foot both sides, size the sponsor equity to balance, prove the check at zero, then common-size each side' },
    accdil:     { family:'E3', board:'table', families:['M4'], name:'Accretion/Dilution',   label:'Run accretion / dilution',   tab:'Acc/Dil',   desc:'Cash, stock or both — pro forma EPS against standalone, drag included' },
    dcfsens:    { family:'M2', board:'grid', name:'Sensitivity',label:'Run the sensitivity table', tab:'Sens.',     desc:'A two-way DCF sensitivity — five discount rates across, three growth rates down' },
    retbridge:  { family:'M4', board:'schedule', families:['M14'], name:'Returns Bridge',label:'Attribute the returns',     tab:'Returns',   desc:'Split the equity gain into growth, multiple and debt paydown — shares of the gain, checked to zero' },
    football:   { family:'M10', board:'table', families:['M4'], name:'Football',  label:'Build the football field',   tab:'Football',  desc:'Six methodologies on one per-share axis — midpoints, implied premiums, and the reference range the board sees' },

    // Formatting group — plus triage/versionup, which live in Formulas II now (groups[] above is authoritative)
    housestyle: { family:'N3', board:'schedule', families:['F1','F2'], name:'House Style', label:'Clean it to standard', tab:'House Style', desc:'A full cleanup pass: title, headers, blue inputs (one buried), commas, %, the totals ruled with a top border' },
    ruleoff:    { family:'F4', board:'schedule', families:['E5'], name:'Rule Off',  label:'Rule off the schedule', tab:'Rule Off',  desc:'Accounting rulings: line under headers, line above every total, outline the headline' },
    ruleaudit:  { family:'F4', board:'schedule', name:'Ruling Pass', label:'The ruling pass', tab:'Ruling Pass', desc:'The page says done \u2014 the pass disagrees. Exactly 4 ruling breaks hide among the house conventions; find all 4, fix only those', errorCount:4 },   /* r425: \u00a72.3 disclosed-error-count prototype \u2014 errorCount puts the found-k/4 meter on the rail */
    combo:      { family:'F1', board:'table', families:['F2','F5','F6'], name:'Combo',    label:'Clean the paste',     tab:'Cleanup',     desc:'Dress a raw pasted comp sheet — weight, alignment, commas, blue ink, wrap and width' },
    typeset:    { family:'F1', board:'form', families:['M11'], name:'Typeset',  label:'Typeset the memo',    tab:'Typeset',     desc:'Bold the header, unbold the imposter, italicize the memos, flag the discontinued line in red, date the page' },   /* r425: language pass — the picker names outcomes, not the answer formula */
    decimals:   { family:'F2', board:'table', families:['N3'], name:'Decimals', label:'The decimals pass',   tab:'Decimals',    desc:'Take a comps page to the house decimal standard, and find the cell somebody hand-formatted' },   /* r425 \u00a74.18 rework: desc matches the 6-beat scope */
    triage:     { family:'M8', board:'schedule', name:'Error triage', label:'Error triage \u2014 #REF! #DIV/0! #VALUE!', tab:'Triage', desc:'Three live errors and eight red cells \u2014 read what each one says, find the causes, rebuild what they were meant to say', errorCount:3 },   /* r439 depth pass (\u00a74.46 + \u00a72.3): errorCount puts the found-k/3 meter on the rail; desc rewritten for the real-sentinel board */
    versionup:  { family:'N4', board:'schedule', families:['M17'], name:'Roll-forward prep', label:'Replace hardcodes so it rolls forward', tab:'Rollfwd', desc:'Every rate is typed inside a formula \u2014 lift them onto the panel so the page rolls forward' },   /* r439 \u00a74.54 depth pass: desc rewritten \u2014 the old one described the retired typed-rows board */
    center:     { family:'F5', board:'table', families:['F4'], name:'Center',   label:'Set the alignment',   tab:'Align',       desc:'Center, left, right — and a long title centered across the table, never merged' },   /* r429: depth-pass rework — the picker names the outcomes the drill now grades (no chord tokens, C5; length keeps the generated meta description inside the 158-char cut) */
    ...
    gauntlet:   { family:'F1', board:'two-block', families:['F2','F4','F6','M1'], name:'Gauntlet', label:'Make it model-ready', tab:'Model',       capstone:true, desc:'The Formatting capstone — a raw sources & uses taken to book standard: blue inputs, live totals, ruled and dollared, columns that fit. One clean run opens the next track leg' },

    // Values-hygiene pair — both live in Data & Lookups now
    drill:      { family:'M18', board:'table', families:['E4'], name:'Hardcode', label:'Hardcode it',         tab:'Hardcode',    desc:'A snapshot still wired into the model — flatten what ships, mark it, archive it, then delete the feed and watch which column survives' },   /* r438 (§4.42): desc described the retired two-check board; rewritten to the prove-out drill */
    series:     { family:'E5', board:'grid', families:['M11'], name:'Series',   label:'Serve the year table',tab:'Years',       desc:'A plan page that came back with its frame stripped — rebuild the year header and the line numbers off their seeds, dress the run, and set the components under their heading' },   /* r438 (§4.43 + delta D15): label rename 'Stub the year header' -> 'Serve the year table', desc rewritten to the reworked board */

    // Data
    sort:       { family:'E8', board:'tape', families:['M1'], name:'Sort',     label:'Sort the table',tab:'Sort',       desc:'A pipeline log ranked by size, a seventh deal signed late — rank it, land the newcomer, rank it again, total and close the line' },   /* r436 §4.33 depth pass: the old desc described the retired 3-check board */
    scrub:      { family:'E2', board:'tape', families:['E6','E8'], name:'Scrub',    label:'Clean the export',     tab:'Scrub',      desc:'A system export with a duplicate header, a page-break row, and a stale SUBTOTAL \u2014 delete all three, sort, re-total' },
    recon:      { family:'M7', board:'two-block', families:['M6','M14'], name:'Recon',    label:'Two systems, one truth',tab:'Recon',     desc:'COUNTIF for presence, INDEX/MATCH for amounts \u2014 drive \u0394 to zero' },
    filterpass: { family:'E9', board:'list', families:['M10'], name:'Filter',   label:'Work the filtered view', tab:'Filter',    desc:'Two screens off one pipeline \u2014 narrow the list, read what is left' },
    unhide:     { family:'E7', board:'table', families:['F6'], name:'Unhide',   label:'Unhide the rows',  tab:'Unhide',    desc:'An inherited regional tape with two regions\u2019 detail hidden outright \u2014 no \u229e, no trace, and the totals summing rows nobody can see. Put the detail back, group every region so the next reader can find it, fold the summary clean and leave the one region the reviewer asked about open' },   /* r437 (DEPTH_PASS \u00a74.37 + the \u00a74.35 merge): the desc named a single hidden span and a width fix; the reworked board is a three-region outline rebuild. name/label/tab unchanged \u2014 the key and its picker identity are immutable (PBs, boards, the generated drills/unhide.html SEO page) */

    // Formulas & auditing — spans Formulas I and Formulas II, plus the Models drills
    margin:     { family:'M4', board:'table', families:['F2'], name:'Margins',  label:'Margins across the page', tab:'Margin',  desc:'Three ratio columns on one peer page — the margin, the growth rate and the EV/EBITDA multiple, each in the format it is quoted in' },   /* r433 §4.21: the desc named the retired three-table board */
    anchor:     { family:'M2', board:'grid', name:'Anchors',  label:'Pin the reference',       tab:'Anchors', desc:'Cycle the reference locks — one pinned formula prices the whole grid' },
    wacc:       { family:'M10', board:'table', families:['M2'], name:'WACC',     label:'Build the discount rate', tab:'WACC',    desc:'Unlever a five-comp beta set, relever at your own structure, weight both sides' },
    fcfbuild:   { family:'E5', board:'schedule', families:['M1','M16'], name:'uFCF',     label:'Build the unlevered FCF', tab:'uFCF',    desc:'EBIT → less taxes → NOPAT → plus D&A, less capex and the NWC build: the row every DCF discounts, filled across five plan years and totalled' },
    dcf:        { family:'M15', board:'schedule', families:['M2'], name:'DCF',      label:'Discount the cash flows', tab:'DCF',     desc:'Discount factors \u00d7 free cash flows build the present-value row; a Gordon terminal value and the enterprise value close the page' },
    dcfbuild:   { family:'M15', board:'schedule', families:['M2'], name:'DCF page', label:'Build the DCF page',      tab:'DCF page',desc:'The valuation page a deal book opens on \u2014 discount five years of free cash flow, add a terminal value, bridge enterprise value down to a price per share, then prove the whole present-value row with one check line' },   /* r449 §4.84 depth pass: desc rewritten — the old one led with three bare abbreviations and named the answer function */
    lbobuild:   { family:'M15', board:'schedule', families:['M14'], name:'Paper LBO',label:'Build the paper LBO',     tab:'Paper LBO',desc:'Size the deal at close, mark the same page at exit, and read the sponsor return \u2014 the equity cheque is the plug that balances the funding' },   /* r449 §4.85 depth pass: desc rewritten — the old one named =IRR() outright and described the retired 25-row S&U board */
    opmodel:    { family:'M3', board:'schedule', families:['M4'], name:'Op model', label:'Build the operating model',tab:'Op model',desc:'The commercial plan lands as units and a price — build five plan years off a four-cell driver panel, take the top line down to EBITDA on a per-unit cost, and leave the margin memo showing which driver is doing the work' },   /* r449 (§4.83): the old line described the retired board — a revenue-growth rate and a gross-profit line, neither of which exists now */
    debtblock:  { family:'M13', board:'schedule', families:['M5'], name:'Debt block',label:'Build the debt & interest block',tab:'Debt block',desc:'A term loan and a revolver, each priced on its own rate — both roll-forwards, interest on the opening balances, then total debt and total interest' },   /* r449 §4.86 depth pass: the old desc named tranches the reworked board does not have. name/label/tab unchanged — the key and its picker identity are immutable (PBs, boards, drills/debtblock.html). */
    dashcover:  { family:'M17', board:'cover', families:['E3'], name:'Model cover',label:'Build the model cover',  tab:'Model cover',desc:'Page one of the pack: wire the headline metrics off the model outputs strip for both cases, then stamp the deal name onto the title' },   /* r449 §4.87 depth pass: the old desc named the answer functions outright — the r417 Class-A leak */
    lbo:        { family:'M15', board:'table', families:['M4'], name:'LBO',      label:'Run the LBO math',        tab:'LBO',     desc:'One entry multiple, three exit years — the equity bridge across all four, then MOIC and IRR per case' },   /* r446 §4.70 depth pass: desc rewritten to the case-column board (the old one described the retired one-column worked example) */
    ...
    modeltour:  { family:'M1', board:'schedule', families:['M4','F2'], name:'Model Tour', label:'Run the model tour', tab:'Model Tour', capstone:true, desc:'The Foundations capstone \u2014 four subtotals blown to #REF! in a live P&L: rebuild the cascade, land both margin rows, dress the bottom line. One clean run opens the next track leg' },
    revolver:   { family:'M5', board:'schedule', families:['M13'], name:'Revolver', label:'Sweep the revolver',  tab:'Revolver',    desc:'Draw, sweep and roll one revolver across four years' },   /* r446 (DEPTH_PASS §4.71): the desc named the retired 7-row board, which had no roll and no draw. De-hint clean — no chord names, no cell refs. */
    cagr:       { family:'M4', board:'schedule', families:['E5'], name:'CAGR',     label:'Compound it, three times', tab:'CAGR',   desc:'Compound each segment’s own rate, then read which one actually grew fastest' },   /* r435 (DEPTH_PASS §4.26): the desc named the retired board ("three scattered blocks") and spelled the answer out as a formula; the reworked drill is a segment revenue build whose second half is a read. Key is immutable, so PBs, boards and runs history are untouched. */
    sumif:      { family:'M6', board:'tape', families:['M4'], name:'SUMIF',    label:'Roll up the segments',tab:'SUMIF',       desc:'SUMIF rollup + live foot + % of total, summary formatted' },
    rollup:     { family:'M6', board:'two-block', families:['M2','M14'], name:'SUMIFS',   label:'Sum on two criteria',      tab:'SUMIFS',      desc:'Roll a booking ledger into a segment \u00d7 region cross-tab, then prove it back' },
    fxconvert:  { family:'M2', board:'two-block', name:'FX Convert', label:'Convert the page \u2014 one anchored rate', tab:'FX', desc:'One blue, boxed rate drives every converted figure \u2014 a locked driver broadcast across the whole table' },
    cases:      { family:'M12', board:'schedule', name:'Sticky switch', label:'Scenario switch with CHOOSE', tab:'Cases', desc:'One switch drives the page \u2014 a driver block repositions every assumption, and a self-referencing IF snapshots each case into the output table' },
    bridge:     { family:'M3', board:'schedule', name:'Point Mode', label:'Point-mode formulas', tab:'Point',   desc:'Roll revenue forward off the growth line, then price EBITDA off the margin line' },   /* r434 (DEPTH_PASS D10): the meta said "Build the profit bridge / profit = revenue × margin", which named neither the drill's lesson (point mode — index.html has said so for revisions) nor its board (a five-year driver build). Renamed per the delta table; key is immutable, so PBs, boards and runs history are untouched. */
    foot:       { family:'M1', board:'table', families:['M14'], name:'Foot',     label:'Total it both ways',  tab:'Cross-foot',  desc:'SUM across and down, tie out the corner' },
    balance:    { family:'M14', board:'schedule', families:['M1'], name:'Balance',  label:'Make it balance',     tab:'Balance',     desc:'Three years of balance sheet with neither side totalled \u2014 foot the assets, foot the liabilities and equity, and build the check row that proves they agree' },   /* r440 depth pass (\u00a74.55): the desc named the retired two-year board and described the work as formatting; the reworked drill is a three-year build whose subject is the two-sided identity. Key is immutable, so PBs, boards and runs history are untouched. */
    audit:      { family:'M9', board:'schedule', families:['N3'], name:'Review Pass', label:'Review pass \u2014 find what\u2019s broken', tab:'Audit', desc:'A divisional review page came back with four breaks in it \u2014 a short Total, two typed-over cells hiding in the calc block, one margin reading the wrong year', errorCount:4 },   /* r439 depth pass (\u00a74.45 + \u00a72.3): absorbs `hunt` \u2014 errorCount puts the found-k/4 meter on the rail; desc rewritten (it described the retired three-break board) */
    balcheck:   { family:'M14', board:'schedule', name:'Make It Tie', label:'Make it tie \u2014 hunt the break', tab:'Make It Tie', desc:'A four-year balance sheet with three breaks in it \u2014 the check row pasted over with zeros, one Total assets starting a row short, one equity cell typed where the roll-forward belongs', errorCount:3 },   /* r440 depth pass (\u00a74.48 + \u00a72.3): errorCount puts the found-k/3 meter on the rail; desc rewritten to disclose the count the prompt now states */
    stalelink:  { family:'M17', board:'schedule', name:'Stale Links',label:'Re-point the stale links', tab:'Stale', desc:'Assumptions moved to v2 \u2014 three cells still read the dead block', errorCount:3 },   /* r439 \u00a74.49 depth pass: \u00a72.3 disclosed-error meter (0/3) */
    tieout:     { family:'M14', board:'table', families:['M1','M17'], name:'Tie-out',  label:'Collapse the suspect leg',   tab:'Tie-out', desc:'Eight legs behind a printed figure and one still reads last quarter \u2014 foot the block, collapse a leg to the number it actually pulls, repoint the stale one and run the tie-out' },   /* r440 depth pass (\u00a74.51): the desc named a three-line board; the reworked one is an eight-leg support block the player foots and ties out. Key is immutable. */
    wrapfix:    { family:'M8', board:'table', families:['M7'], name:'IFERROR',  label:'Wrap it or fix it',   tab:'IFERROR', desc:'Three reads down for three different reasons \u2014 wrap the truly missing, repair the merely broken, then foot the panel' },
    signerr:    { family:'M16', board:'schedule', name:'Sign Sweep', label:'Flip the signs back', tab:'Signs', desc:'One plan year came in on the other sign convention \u2014 find it, flip it, re-lay EBIT', errorCount:3 },   /* r439 \u00a74.53 depth pass: \u00a72.3 disclosed-error meter (0/3); desc rewritten for the rebuilt board */
    percent:    { family:'M2', board:'two-block', families:['M4'], name:'% of Revenue', label:'Common-size both statements', tab:'Common-size', desc:'Both blocks ÷ their OWN revenue, $-locked so the fill can’t drift' },
    intsched:   { family:'M13', board:'schedule', families:['M5'], name:'Interest', label:'Run the interest schedule', tab:'Interest', desc:'A term loan that opens where it closed, cash interest on the beginning balance, and the coverage line the credit committee reads' },   /* r446 §4.69 depth pass: desc rewritten to the reworked board (the old one narrated the mechanism — §1.0(b) puts that in guide/req, never player copy) */
    schedule:   { family:'M13', board:'schedule', name:'Schedule', label:'Roll it forward',     tab:'Schedule',    desc:'Five years of fixed assets — capex adds, depreciation takes its cut, and every year opens where the last one closed, with the accumulated-depreciation memo underneath' },   /* r446 §4.68 depth pass: desc rewritten to the reworked board (the old one described a ROWS:9 page with no capex split and no assumption stack) */
    comps:      { family:'M10', board:'table', families:['F3'], name:'Comps',    label:'Run the comps',       tab:'Comps',       desc:'Build both enterprise-value multiples across the peer set, read the median, high and low, and carry it down to an implied share price' },

    // Full Builds — plus lookup/lookup2, which live in Data & Lookups
    isbuild:    { family:'M3', board:'schedule', families:['M4'], name:'IS Build',  label:'Build the income statement', tab:'IS Build',  desc:'Five forecast years off one driver panel — revenue grown year on year, the cost lines as a percent of it, EBITDA ruled and the margin memo underneath' },   /* r448 §4.78 depth pass: desc rewritten to the reworked board (the old one described an 11-row page with a tax line and a net-margin row, and named the mechanism rather than the artifact) */
    bsbuild:    { family:'M14', board:'schedule', families:['M13'], name:'BS Build',  label:'Balance the balance sheet',  tab:'BS Build',  desc:'Three years: roll retained earnings, total both sides, land the check at zero' },   /* r448 §4.79 depth pass: desc rewritten to the reworked board (the old one named the mechanism — SUM, fill — not the artifact) */
    nwcsched:   { family:'M4', board:'schedule', families:['M13'], name:'NWC Schedule',label:'Roll working capital',       tab:'NWC',       desc:'Roll receivables, inventory and payables off three day-count drivers' },   /* r448 §4.81 depth pass: desc rewritten to the reworked board (the old one narrated the route — type/paint — not the artifact) */
    threestmt:  { family:'M17', board:'two-block', families:['M14'], name:'3-Statement',label:'Tie the three statements',  tab:'3-Stmt',    desc:'Three statements on one page — net income, cash and retained earnings wired, the check at zero' },   /* r448 §4.82 depth pass: desc rewritten to the reworked board (the old one advertised checks plural and pre-shipped dress) */
    cfslink:    { family:'M17', board:'schedule', families:['M13'], name:'CFS Link',  label:'Link the cash flow statement',tab:'CFS Link', desc:'Point the statement at its schedules, close the cash roll year to year, read cash conversion against EBITDA' },   /* r448 §4.80 depth pass: desc rewritten to the reworked board — the old one narrated the dress, and the board has a supporting-schedule block now */
    debtsched:  { family:'M13', board:'schedule', families:['M5'], name:'Debt Schedule',label:'Run the debt schedule',      tab:'Debt',     desc:'Roll the term loan year to year — mandatory amortization, a capped cash sweep, interest on the beginning balance' },   /* r447 §4.76 depth pass: desc rewritten to the reworked board (the old one narrated the route — type/paint — not the artifact) */
    lookup:     { family:'M7', board:'two-block', name:'Lookup',   label:'Look it up',          tab:'Lookup',      desc:'Read a peer table with INDEX/MATCH \u2014 three pulls onto one pitch screen' },
    lookup2:    { family:'M7', board:'grid', families:['M2'], name:'Two-way Lookup',    label:'Two-way lookup',      tab:'2-way',       desc:'INDEX with two MATCHes — row and column at once' },
  },
};
...
window.HOTKEY_ACHIEVEMENTS = [
  ...
  { id:'x_tour',   glyph:'explorer', name:'Tourist',            desc:'Attempt 10 different drills',            test:c=>({done:c.att>=10, prog:Math.min(c.att,10), goal:10}) },
  { id:'x_comp',   glyph:'explorer', tier:'e', name:'Completionist',      desc:'Attempt every drill on the board',       test:c=>({done:c.att>=c.menuOrder.length, prog:c.att, goal:c.menuOrder.length}) },
  { id:'x_sub5',   glyph:'speed', tier:'r', name:'Blink',             desc:'Finish any drill in under 5 seconds',    test:c=>{ const ok=(c.runs||[]).some(r=>r.time_ms<5000); return {done:ok, prog:ok?1:0, goal:1}; } },
  { id:'x_zero',   glyph:'perfect', tier:'r', name:'No Wasted Keys',    desc:'Match the optimal keystroke count exactly', test:c=>{ const ok=(c.runs||[]).some(r=>r.optimal&&r.keystrokes===r.optimal); return {done:ok, prog:ok?1:0, goal:1}; } },
  ...
  { id:'x_dc10',  glyph:'daily', tier:'r', name:'Daily Contender',  desc:'Finish top 10 in a Daily Challenge',      test:c=>({done:(c.dailyTop10||0)>=1, prog:Math.min(c.dailyTop10||0,1), goal:1}) },
  { id:'x_dc10x5',glyph:'daily', tier:'e', name:'Daily Fixture',    desc:'Finish top 10 in five Daily Challenges',  test:c=>({done:(c.dailyTop10||0)>=5, prog:Math.min(c.dailyTop10||0,5), goal:5}) },
  /* r372: the full daily placement ladder — top 10 → podium → win → dynasty */
  { id:'dc_pod',  glyph:'crown', tier:'r', name:'Daily Podium',     desc:'Finish top 3 in a Daily Challenge',       test:c=>({done:(c.dailyPod||0)>=1, prog:Math.min(c.dailyPod||0,1), goal:1}) },
  { id:'dc_win',  glyph:'crown', tier:'e', name:'Daily Champion',   desc:'Win a Daily Challenge — #1 on the day', test:c=>({done:(c.dailyWins||0)>=1, prog:Math.min(c.dailyWins||0,1), goal:1}) },
  { id:'dc_win5', glyph:'crown', tier:'m', name:'Daily Dynasty',    desc:'Win five Daily Challenges',               test:c=>({done:(c.dailyWins||0)>=5, prog:Math.min(c.dailyWins||0,5), goal:5}) },   /* r376: promoted to mythic */
  /* r372: certificate feats — the tracks' capstones show on the wall too */
  { id:'cert1',   glyph:'cert',  tier:'e', name:'Certified',        desc:'Earn your first certificate',             test:c=>({done:(c.certs||0)>=1, prog:Math.min(c.certs||0,1), goal:1}) },
  { id:'cert3',   glyph:'cert',  tier:'l', name:'Triple Crown',     desc:'Earn all three certificates',             test:c=>({done:(c.certs||0)>=3, prog:Math.min(c.certs||0,3), goal:3}) },
  ...
  { id:'vol0',  glyph:'volume',  name:'Opening Bell',    desc:'25 clean solves',                        test:c=>({done:c.solves>=25, prog:Math.min(c.solves,25), goal:25}) },
