...
window.HOTKEY_ACHIEVEMENTS = [
  ...
  { id:'day2', glyph:'day', tier:'e', name:'Fixture',          desc:'Run 50 daily challenges',               test:c=>{ const n=new Set(c.runs.map(r=>r.challenge).filter(ch=>/^(challenge|daily)-/.test(ch||''))).size; return {done:n>=50, prog:Math.min(n,50), goal:50}; } },
  ...
  { id:'day0',  glyph:'day',  name:'Morning Person',  desc:'Run 3 daily challenges',                 test:c=>{ const n=new Set(c.runs.map(r=>r.challenge).filter(ch=>/^(challenge|daily)-/.test(ch||''))).size; return {done:n>=3, prog:Math.min(n,3), goal:3}; } },
