...
window.HOTKEY_ACHIEVEMENTS = [
  ...
  { id:'day1', glyph:'day', name:'Regular',          desc:'Run 10 daily challenges',               test:c=>{ const n=c.runs.filter(r=>/^(challenge|daily)-/.test(r.challenge||'')).length; return {done:n>=10, prog:Math.min(n,10), goal:10}; } },
  { id:'day2', glyph:'day', tier:'e', name:'Fixture',          desc:'Run 50 daily challenges',               test:c=>{ const n=c.runs.filter(r=>/^(challenge|daily)-/.test(r.challenge||'')).length; return {done:n>=50, prog:Math.min(n,50), goal:50}; } },
  ...
  { id:'day0',  glyph:'day',  name:'Morning Person',  desc:'Run 3 daily challenges',                 test:c=>{ const n=c.runs.filter(r=>/^(challenge|daily)-/.test(r.challenge||'')).length; return {done:n>=3, prog:Math.min(n,3), goal:3}; } },
