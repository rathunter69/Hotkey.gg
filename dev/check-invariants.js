/* STATIC INVARIANTS GUARD (Segment C, r414-review). Catches the single-source-of-
   ...
   diverging (caused the #121 notch-pill bug and the #76 membership drift). No browser.
     ...
     C15 — the Keyboard Tour (r452) is NOT a drill: outside menuOrder/PARS/CLOCKS/POOL/
           PLACEMENT/TRACKS/CAMPAIGN, 24 beats index-paired with 24 targets (exactly one
           null), 5 staged tiers, 6 lesson cards, the HUD's per-beat copy, the runtime
           seams it hangs off, the retired sandbox — and the lesson-drill contract the
           later wave inherits.
...
/* ---- C15 (r452, dev/TUTORIAL_CHAPTER_SPEC.md §7): THE KEYBOARD TOUR IS NOT A DRILL ----
...
try {
  ...
  if (menu.includes(KT)) bad(`C15: '${KT}' is in drills.js menuOrder — the Tour is not a drill (§2(g))`);
  for (const [name, map] of [['HOTKEY_PARS', W15.HOTKEY_PARS], ['HOTKEY_CLOCKS', W15.HOTKEY_CLOCKS]])
    if (map && KT in map) bad(`C15: '${KT}' is in ${name} — it must be impossible to time the Tour (§7)`);
  if ((W15.HOTKEY_CHALLENGE_POOL || []).includes(KT)) bad(`C15: '${KT}' is in HOTKEY_CHALLENGE_POOL`);
  if (((W15.HK_PLACEMENT || {}).KEYS || []).includes(KT)) bad(`C15: '${KT}' is in HK_PLACEMENT.KEYS`);
  for (const t of Object.keys(W15.HK_TRACKS || {}))
    if ((W15.HK_TRACKS[t].keys || []).includes(KT)) bad(`C15: '${KT}' is in HK_TRACKS.${t}.keys`);
  for (const c of ((W15.HOTKEY_CAMPAIGN || {}).chapters || []))
    if ((c.keys || []).includes(KT)) bad(`C15: '${KT}' is in HOTKEY_CAMPAIGN chapter ${c.id}`);
  for (const g of (D15.groups || []))
    if ((g.keys || []).includes(KT)) bad(`C15: '${KT}' is in drills.js group '${g.name}'`);
  if ((D15.meta || {})[KT]) bad(`C15: drills.js meta.${KT} exists — the Tour has no picker entry (§2(g))`);
  ...
  if (!/filter\(k\s*=>\s*!g\.includes\(k\)\s*&&\s*!CHALLENGES\[k\]\.tour\)/.test(idx15))
    bad('C15: MENU_ORDER\'s fallback no longer filters `tour:true` entries — keyboardtour would ' +
  ...
  if (!tc) bad(`C15: CHALLENGES.${KT} not found — the Keyboard Tour board is gone`);
  else {
    if (!/\btour\s*:\s*true/.test(tc)) bad(`C15: ${KT} must declare tour:true (that flag is what keeps it out of MENU_ORDER)`);
    if (/\bpar\s*:/.test(tc) || /\bparKeys\s*:/.test(tc))
      bad(`C15: ${KT} declares a par — it must be impossible to time the Tour (§7)`);
    if (/saveClose\s*:\s*true/.test(tc))
      bad(`C15: ${KT} declares saveClose — hkSaveCloseWire would append a 25th beat over stage 6's own save`);
    const ci = tc.indexOf('checks(');
    const nChecks = ci >= 0 ? (tc.slice(ci).match(/\{label:/g) || []).length : 0;
    if (nChecks !== 24) bad(`C15: ${KT} has ${nChecks} beats, expected 24 (§3.0.5: 5·4·5·4·5·1)`);
    ...
    if (!tm) bad(`C15: ${KT} targets() is not a literal array (the C9 static parser reads it as one)`);
    else {
      const nulls = (tm[1].match(/(^|[,\s])null(?=[,\s]|$)/g) || []).length;
      if (nulls !== 1) bad(`C15: ${KT} targets() has ${nulls} null entries, expected exactly 1 (stage 6 beat 1)`);
    }
    /* five staged tiers, one per stage 2-6, every checks index inside 0..23 */
    const tiers = tc.match(/\{ label:'[^']*', checks:\[([0-9,\s]*)\]/g) || [];
    if (tiers.length !== 5) bad(`C15: ${KT} declares ${tiers.length} tiers, expected 5 (stages 2-6, §7)`);
    for (const t of tiers) {
      const ix = /checks:\[([0-9,\s]*)\]/.exec(t)[1].split(',').map(x => parseInt(x, 10)).filter(x => !isNaN(x));
      if (!ix.length) bad(`C15: ${KT} tier ${t.slice(0, 30)} lists no checks`);
      for (const i of ix) if (i < 0 || i >= nChecks)
        bad(`C15: ${KT} tier checks index ${i} is outside its ${nChecks}-beat checks() array`);
    }
    /* six stage cards, each with a title, a body of at most 60 words and a non-empty keycap strip */
    const stagesM = /stages:\s*\[([\s\S]*?)\n    \],/.exec(tc);
    if (!stagesM) bad(`C15: ${KT} declares no stages[] (the six §1.5 lesson cards)`);
    else {
      ...
      if (titles.length !== 6 || bodies.length !== 6 || keys.length !== 6)
        bad(`C15: ${KT} stages: ${titles.length} titles / ${bodies.length} bodies / ${keys.length} keycap strips, expected 6 of each`);
      bodies.forEach((b, i) => {
        const words = b.slice(6, -1).split(/\s+/).filter(Boolean).length;
        if (words > 60) bad(`C15: ${KT} stage ${i + 1} card body is ${words} words — the §1.5 lesson card caps at 60`);
      });
    }
  }
  ...
  if (!bm) bad('C15: HK_TOUR_BEATS not found — the TUTORIAL HUD has no per-beat copy (§3.0.2)');
  else {
    const huds = (bm[1].match(/hud:'((?:[^'\\]|\\.)*)'/g) || []).map(x => x.slice(5, -1));
    const nudges = (bm[1].match(/nudge:'((?:[^'\\]|\\.)*)'/g) || []).map(x => x.slice(7, -1));
    if (huds.length !== 24) bad(`C15: HK_TOUR_BEATS carries ${huds.length} hud lines, expected 24`);
    if (nudges.length !== 24) bad(`C15: HK_TOUR_BEATS carries ${nudges.length} nudge lines, expected 24 (every beat gets the three-miss line)`);
    huds.forEach((h, i) => {
      if (!h.trim()) bad(`C15: HK_TOUR_BEATS[${i}].hud is empty — the HUD is the ONLY instruction surface on the Tour`);
      const w = h.split(/\s+/).filter(Boolean).length;
      if (w > 14) bad(`C15: HK_TOUR_BEATS[${i}].hud is ${w} words — §3.0.2(5) caps the banner at 14 ("${h.slice(0, 50)}")`);
      if (i && h === huds[i - 1]) bad(`C15: HK_TOUR_BEATS[${i}].hud repeats beat ${i - 1}'s line — one beat, one line`);
    });
  }
  ...
  for (const [re, why] of seams) if (!re.test(idx15)) bad('C15: ' + why);
  ...
  for (const dead of ['function startSandbox', 'function sandboxReadyCard', 'function sandboxCallout',
    ...
    if (idx15.includes(dead))
      bad(`C15: '${dead}' is back in index.html — the warm-up sandbox was retired into the Keyboard Tour (§1.7, T2)`);
  if (!/let sandboxMode=false;/.test(idx15))
    bad('C15: sandboxMode was deleted — §8 do-not-change #6 keeps the FLAG (checkWin, updateChecklist and loadChallenge all read it)');
  if (fs.existsSync('dev/e2e-onboard-sandbox.js'))
    bad('C15: dev/e2e-onboard-sandbox.js is back — it tests a surface that no longer exists (§1.7)');
  ...
  for (const k of lessonKeys) if (!metaLesson.includes(k))
    bad(`C15: CHALLENGES.${k} carries a lesson but drills.js meta.${k}.lesson is not true (§2(a): the two must never drift)`);
  for (const k of metaLesson) if (!lessonKeys.includes(k))
    bad(`C15: drills.js meta.${k}.lesson is true but CHALLENGES.${k} declares no lesson object (§2(a))`);
  for (const k of lessonKeys) {
    ...
    if (!title || !bodyL || !keysL || !keysL[1].trim())
      bad(`C15: ${k}.lesson needs title, body and a non-empty keys strip (§1.5)`);
    else {
      const w = bodyL[1].split(/\s+/).filter(Boolean).length;
      if (w > 60) bad(`C15: ${k}.lesson.body is ${w} words — the lesson card caps at 60 (§1.5)`);
    }
    if (!/reveal\s*:\s*true/.test(ch))
      bad(`C15: lesson drill ${k} must render its ☆ visibly (reveal:true) — the efficiency IS the lesson (§1.6)`);
    if (!menu.includes(k)) bad(`C15: lesson drill ${k} is not in menuOrder — lesson drills ARE drills (§2(h))`);
    if ((W15.HOTKEY_CHALLENGE_POOL || []).includes(k)) bad(`C15: lesson drill ${k} is in HOTKEY_CHALLENGE_POOL — a ≤30 s board is not a Daily Challenge (§4 A5)`);
    if (((W15.HK_PLACEMENT || {}).KEYS || []).includes(k)) bad(`C15: lesson drill ${k} is in HK_PLACEMENT.KEYS (§7)`);
    const par = (W15.HOTKEY_PARS || {})[k];
    if (!(par > 0 && par <= 30)) bad(`C15: lesson drill ${k} par is ${par} — lesson pars are ≤ 30 s (§7)`);
    const cl = (W15.HOTKEY_CLOCKS || {})[k];
    if (!cl || Math.abs(cl.pass - par * 2) > 0.01)
      bad(`C15: lesson drill ${k} clocks: pass must be par×2.0 (§2(b)), got ${cl && cl.pass} against par ${par}`);
  }
  ...
  for (const k of Object.keys(chunks15))
    if (/reveal\s*:\s*true/.test(chunks15[k]) && !lessonKeys.includes(k))
      bad(`C15: ${k} declares reveal:true but is not a lesson drill — the ☆ is a hidden discovery everywhere else (§1.6)`);
  if (!/function hkLessonKey\(k\)\{/.test(idx15))
    bad('C15: hkLessonKey() is gone — §2(a) names it as the single lesson-drill helper');
  ...
  bad('C15 could not run: ' + String(e.message || e).slice(0, 160));
}
