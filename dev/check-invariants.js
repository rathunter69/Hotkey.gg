...
/* ---- C14 (r452): the Mac chord truth table is SSOT ----
...
try {
  ...
  if (!TABLE.test(themes)) bad('C14: themes.js no longer defines window.HK_MAC_CHORDS (the Mac truth table)');
  else if (rows < 12) bad(`C14: HK_MAC_CHORDS has only ${rows} rows — the Mac exception table looks gutted`);
  ...
  for (const fn of ['hkMacSpec', 'hkMacNote', 'hkMacChord', 'hkMacLookup'])
    if (!new RegExp('window\\.' + fn + '\\s*=').test(themes)) bad(`C14: themes.js no longer exports ${fn}()`);
  if (!/window\.hkMacSpec/.test(ref) || !/window\.hkMacNote/.test(ref))
    bad('C14: reference.html no longer derives its Mac column from themes.js (hkMacSpec/hkMacNote)');
  ...
  if (localSwap.test(ref) || /function\s+macCap\s*\(/.test(ref))
    bad('C14: reference.html has its own ctrl→⌘ swap again — that is the drift this guard exists for');
