...
/* ---- C14 (r452): NO `name:` / `label:` KEY INSIDE A CHALLENGES BLOCK ----
...
try {
  ...
  const start = idx.indexOf('const CHALLENGES = {');
  const end = idx.indexOf('STATE + ENGINE', start);
  if (start < 0 || end < 0) bad('C14: CHALLENGES block not found in index.html (shape changed?)');
  else {
    const body = idx.slice(start, end);
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (hit) {
        n14++;
        bad(`C14: index.html:${base + i} CHALLENGES.${key} carries an inline \`${hit[1]}:\` — drills.js meta is the SSOT for display strings (drills.js header, lines 8-10); delete it, don't sync it`);
      }
    }
...
/* ---- C15 (r452): CAMPAIGN CHAPTER NAMES ARE GROUP NAMES ----
...
try {
  ...
  if (!groups15.length || !camp15.length) bad('C15: groups[] or HOTKEY_CAMPAIGN.chapters did not parse (shape changed?)');
  else {
    let n15 = 0;
    for (const c of camp15)
      if (!groups15.includes(c.name)) {
        n15++;
        bad(`C15: HOTKEY_CAMPAIGN chapter ${c.id} name '${c.name}' is not a groups[] name — the picker folder and the campaign rail must show one string (put any suffix in \`sub\`). Groups: ${groups15.join(' | ')}`);
      }
