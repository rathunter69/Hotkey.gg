...
### 1.0-R2 PLAYTEST ROUND-2 LAW (r420d, Wolf 2026-07-25 — BINDING, same force as §1.0; §1.0-R2 wins over any older line)
Source: dev/ROUND2_FEEDBACK.md (Wolf's round-2 playtest of the wave-1 rebuilds). Seven rules, catalog-wide; letters continue §1.0's:

**(g) ADAPTIVE CHECKLIST LABELS.** When a beat's required action VARIES PER SEED, the label states THIS seed's action concretely — "Advisory is the only row in thousands — divide by 1000", "Trading is in hundreds — multiply by 10" — computed at build time from the seed's parameters. WHY (Wolf, 2026-07-25): "I'm looking at the item row and trying to figure out what conversion I need to do" — a static label that forces the player to reverse-engineer the seed is a SPEC BUG, not a difficulty feature. Implementation: build() interpolates the seed facts into the check-label string (the §2.3 live-counter precedent). Boundary with §1.0(b): the label stays outcome-vague about the ROUTE (never names the chord or the mechanism) while being precise about the SEED FACT (which row, which unit, which conversion). Wrong: "Convert the inconsistent row to dollars." Right: "Advisory is the only row in thousands — divide by 1000." (The route — helper cell, paste-special-multiply, or slow typing — remains the player's choice, §1.0(c).)

**(h) ACTION-LOCATION CUES.** Where the player must act at a SPECIFIC SPOT (insert here, paste here), the BOARD carries an explicit cue — e.g. red "<<< Insert row above" text in a spare column beside the site — not just the subtle guided highlight. WHY (Wolf, 2026-07-25, rowops): "make sure it's very very clear where you have to insert the row — like red '<<< Insert row above' text in the right column vs. the subtle green highlight." Rules: cues are BOARD CONTENT — seeded by build() at the seed's site, styled loud (red ink), and REMOVED or IGNORED by grading (a cue cell is never a graded target, and whether the player deletes it or a structure op moves it must not affect any check's ok). Distinct from guided mode: the cue is always-on board furniture for everyone; the guided ring remains the opt-in rail.

**(i) ☆ MUST BE A CHOICE (extends §1.0(d)).** A bonus that is earned "by the nature of doing the exercise" is DEAD — the ☆ must require a DISTINCT, SKIPPABLE mastery decision, e.g. Ctrl+Shift+↓ to select the data block before copying/cutting instead of arrowing down it. WHY (Wolf, 2026-07-25, exact framing): "same with assemble and format, you achieve the star by the nature of doing the exercise. Should maybe be … that you used ctrl arrow down to select the data before copying or cutting vs using arrow keys — basically reward for good habits of using the previously taught hotkeys that save time." Page-author test: can a player clear every core beat WITHOUT earning the ☆ and without going out of their way? If the ☆ falls out automatically, it is a spec bug. (The wave-1 round-2 blocksel and rowops ☆s failed this test — re-cut in their round 3.)

**(j) LANGUAGE — "re-tie"/"tie" BANNED for formula recalculation (extends §1.7 R7).** In finance, TYING means the numbers MATCH or BALANCE — a tie-out, a tie check reading zero, a schedule that ties to the statement (those usages remain legal). A SUM adapting to an insert/delete/cut is "the total recalculates" (or calculates) — NEVER "re-ties". WHY (Wolf, 2026-07-25): "Stop saying totals 're-tie' — the totals calculate or recalculate. … when you tie numbers in finance you're making sure they match or things balance, not just like, watching the sum formula adapt to a cut or deletion." Added to the R7 banned class with this gloss; this doc's own pages were swept in r420d (§1.6, §4.5, §4.7, §4.53). Sweep rule for wave agents: "tie" describing a MATCH stays; "tie"/"re-tie" describing RECALCULATION is rewritten.

**(k) FIRST-DRILL NUDGE (sanctioned exception).** Drill 1 (navigation) MAY use a flashing indicator/arrow to nudge the very first movement through the corridor. WHY (Wolf, 2026-07-25): "for the very first drill we might want some flashing indicator or arrow button to just nudge the user to move through the corridor." This is an explicit, ONBOARDING-SCOPED exception to the no-hand-holding default (§1.0(b)): it applies to the FIRST drill only, to the FIRST action only, and disappears once the player moves. No other drill inherits it — a nudge anywhere else in the catalog is a spec bug.
...
**(r) ENGINE FACTS THAT CHANGED UNDER YOU (r429–r431).** Pages written before r429 may assume the old behaviour:
  · **All four border edges now render — as of r442, not r429.** This bullet claimed the fix twice before it was true, so here is the measured state: r429 gave every edge a CSS rule and made applied borders 2px, but `render()` still carried r292's INLINE emitter writing `border-*: 1px` for `bl`/`br`/`thick`, and inline beats the stylesheet. Left painted **0.00px**, right a **1.00px** hairline, and the thick box was identical to the thin one; only top and bottom ever worked. r442 deleted the emitter and moved applied borders onto an overlay pseudo-element. Verified: every edge 2.00px, thick 3.00px, double two 1px strokes. Outside borders are now genuinely safe to use and to grade.
  · **`dev/check-borders.js` and `dev/check-pause.js` are gate steps** (they existed from r429 but ran on no PR until r442). If you grade a border, that guard is what protects you.
  ...
  · **The #### test reasons in UNSCALED px.** It used to compare against the elastic display width, so the fit-scaler manufactured #### on columns that were genuinely wide enough. `overflowsCol()` (which grades autofit beats) matches it.
  · **WIDTH BEATS GRADE THE BOARD, NOT AUTOFIT'S TARGET (r441 — Wolf's ruling; supersedes anything a pre-r441 page says about width).** There are exactly two width verdicts and you grade one of them, never a width number:
    · figures — **`!overflowsCol(S,c)`**: no `####` left in the column;
    · labels — **`!clipsCol(S,c)`**: no label amputated (a label that spills into empty neighbours is NOT clipped — Excel clips text and never `####`s it, and so do we).
    Do **not** grade `colW >= neededWidth(c)-1`. `neededWidth()` is what **autofit SETS**, not what the board needs: it carries `FIT_SLACK` past the overflow threshold, so a player who types a width (`Alt H O W`, ~7px per Excel unit) can clear every visible `####` and still land under it — a perfect board with a dark line, §1.0-R3 exactly. Both bands were live before r441 and measured: 4px on `combo`, **21px** on `housestyle`, where the old math sized LABELS with the mono NUMBER metric.
    If you need a width number for a board (not for a beat), call `neededWidth()`; never re-derive a metric from `CHARPX`/`TXTPX`/the pads by hand. `housestyle` shipped a hand-inlined copy and it drifted.
...
### 1.0-R4 THE DUAL-AUDIENCE REAL-TASK LAW (r429, Wolf directive 2026-07-27 — BINDING, same force as §1.0/§1.0-R2/§1.0-R3)

> **RENUMBERED IN r443, CONTENT UNTOUCHED.** This law arrived on PR #244 as "§1.0-R3 (n)/(o)"
> while PR #243 was independently writing a *different* §1.0-R3 (n)–(s) from the round-3
> playtest. Both are binding directives from Wolf and neither supersedes the other, so the
> playtest block keeps §1.0-R3 (n)–(s) — the engine, the drills and the CI lint all cite
> R3(p)/(r)/(s) by letter — and this block becomes **§1.0-R4**, its two laws relettered
> **(n)→(t)** and **(o)→(u)**. Any pre-r443 reference to "§1.0-R3(n) dual-audience" or
> "§1.0-R3(o) ☆ re-cut" means §1.0-R4(t) and §1.0-R4(u). Nothing else about them changed.
...
**(t) EVERY DRILL IS A REAL TASK, FOR BOTH AUDIENCES.**
...
**(u) THE ☆ RE-CUT SWEEP (r429 — catalog-wide known defect, stated once so no agent has to ask).**
...
#### 4.1 navigation — "Navigation maze" · M — **ROUND 3 (playtest r2, r427 — SHIPPED)**
**Round-2 verdict (Wolf):** "honestly very good" — three changes, all landed in r427:
- **Windier corridor** ("more movement and actions; there's a lot of dead space with just a block of black cells"). The ad-hoc leg templates are replaced by a **SERPENTINE generator**: nLeg horizontal legs (4 or 5, biased to 5) at strictly increasing rows alternate between a RIGHT column bound (6–10) and a LEFT one (1–3), each pair joined by a vertical drop → **8 or 10 corridor legs per seed, up from 6–8**, and open cells rise from ~50 to ~69 of 200. Every leg is still exactly ONE Ctrl-arrow flight and `build()` still self-verifies every seed (flights + moats), now over the exit legs too. Geometry laws are written into the build comment (rows[1] ≥ 6 so a LEFT leg never runs flush under the A1 room; leg rows ≥2 apart; block hangs 2 rows under the last leg; block bottom ≤18).
- **First-drill nudge** (§1.0-R2(k)): a blinking chevron in the four cells straight ahead of the start. RENDER-ONLY — no `cells` entry, so it can never touch grading, `usedRange()` or the paste snapshot; `pointer-events:none`; `move()` sets `maze.moved` on the first successful step or flight and it is gone on the next render. Honors `prefers-reduced-motion`.
- **An EXIT** ("add a small exit so you can still get THROUGH the corridor to complete the 'go to last cell' checklist item"): a gap at the block's bottom-right runs down to row 20 and along it to J20 — exactly where Ctrl+End lands — so beat 5 has a legitimate WALKED route (two more flights) as well as the teleport. Registered as navigation ALT #3 (the on-foot route presses no Ctrl+End at all).
**Wall styling HELD** (Wolf offered thick borders as an alternative): the wall CELLS are what put content in row 20 / column J, and Ctrl+End rides `usedRange()` — unpainting them would collapse the used range and silently break beat 5. Density is fixed by carving more corridor. Channel cells already carry the thick `.mzt/.mzb/.mzl/.mzr` tube edges.
**☆ re-verified fair** on the windier layout: `S.bumpN` still counts ONLY a keypress the wall swallowed, post-win presses are excluded by the `!done` guard, and the demo + all three ALTs clear it. **Par re-swept:** parKeys 14 → **17** (measured median ×5 seeds ×3 runs), par 16 → **20** (1.18 s/key).

#### 4.1-r2 navigation — round-2 record (superseded above)
...
#### 4.2 filldr — "Fill down, fill right" · M — **ROUND 3 (playtest r2, r427 — SHIPPED)**
**Round-2 verdict (Wolf):** "good drill" — three board changes, all landed in r427:
- **VARYING quarterly costs.** Wolf: "I wish we had something else to fill right vs straight lining the Q1 numbers — not super intuitive that all of those costs didn't change over the year." Of the two approaches available (formulas off a varying revenue row · seeded per-quarter growth) the build takes **FORMULAS**, because compounded growth lands on non-integer figures a slow-typing player (§1.0(c)) could never match. Q1 now ships as a LIVE formula per cost line and the fill-right TRANSLATES it: **COGS = −(COGS % helper) × that quarter's Revenue**, **Opex = −(Opex % helper) × that quarter's Revenue** (both `$I$`-anchored), **D&A = that quarter's line on the fixed-asset schedule** (a second seeded feed row, relative ref). The revenue feed swings quarter to quarter, so all three cost rows now move. Beat 2 stays exactly ONE fill-right — nothing new to type. **INTEGER GUARANTEE:** feed values are multiples of 20 and the percentage helpers are multiples of 0.05 (= n/20), so pct × feed is always whole — a typed figure clears the same 0.5 tolerance a filled one does. **Ratio-block payoff:** COGS % and Opex % come out flat (they ARE the assumptions) while D&A % and the EBITDA margin % move with revenue — a common-size block that says which costs scale and which don't.
- **The ratio block is a STANDALONE SUMMARY BLOCK**: an EMPTY row now separates it from the EBITDA line above (Wolf: "like summary stats in a model output").
- **Exactly ONE margin row**, labeled **"EBITDA margin %"**. NOTE for the record: the duplicate Wolf saw belonged to the pre-r424 board — the r424 build already carried exactly one. r427 pins it with CI invariant **C10** (the filldr board may carry exactly one row label containing "margin", spelled "EBITDA margin %").
**§1.0-R2(l) helper stack:** the two assumptions ship vertically stacked and individually labeled ("Helper — COGS %" / "Helper — Opex %"), light yellow + all borders, populated, under an "Assumptions" block title (cols H/I) — never graded targets, just the inputs the seeded formulas already point at.
**☆ re-verified** against the new geometry (rows shifted by the D&A feed + the empty separator): the census + fill-down/fill-right latches read off `o.p1`/`o.rr`, both ALTs still pass (slow route clears core without the ☆; AutoSum route earns it). **Par re-swept:** parKeys/par hold at **44/44** — the beat sequence and key count are unchanged.

#### 4.2-r2 filldr — round-2 record (superseded above)
...
#### 4.3 pastes — "Paste Special everything" · M — **ROUND 3 (playtest r2, r427)**
**ROUND-3 DELTA (Wolf playtest round 2, dev/ROUND2_FEEDBACK.md §2 pastes) — this block supersedes the round-2 lines below where they conflict:**
- Beat 1 loses its tail. Wolf: "the Q labels say which quarter is which" was "a comment for you on clarity, not necessarily explicit for the user." The line ships as **"Transpose the fees from the vertical feed into the Fees row"**; the Q-label reading moves to `guide[0]`.
- **§1.0-R2(g) ADAPTIVE LABEL.** The convert beat states THIS seed's conversion, computed in `build()` as `o.convLabel`: `÷1000` seed → "Convert the Trading and Advisory lines — they arrived in raw dollars, divide by 1,000"; `×100` seed → "…they arrived in hundreds, multiply by 100". Precise about the SEED FACT, silent about the ROUTE.
- **§1.0-R2(l) HELPER STACK.** The two helpers are VERTICALLY STACKED with an individual label in column A beside each: "Helper — 1000s"/"Helper — 100s" (unit co-varying with the seed) over "Helper — sign flip". Light yellow + all borders + blue figure; labels are board content, never graded targets. (Border rendering is fixed as of r426.)
- **NEW beat 2 — right-align.** The fees feed arrives LEFT-ALIGNED (source cells and the landing row both), the transpose carries the alignment, and the player right-aligns the fee row. Placed directly behind the transpose (§1.0(a)).
- **NEW beat 7 — outside border around the deck values row**, directly behind the values hand-off that fills it (§1.0(a)). Its guide line teaches **Alt H B S** per §1.0-R2(m) and names H B A as ALL borders.
- **§1.0-R2(j)** — "the Subtotal re-ties on its own" → "the Subtotal RECALCULATES on its own".
- **Beat count: 7 core** (+ the save closer). §1.1 DEVIATION declared in the drill's source header: Wolf's round-2 list adds two beats and drops none.
- **Par re-swept:** parKeys 32→40, par 34→42 (5-seed median, ~1.05 s/key). **ALTS:** both entries carry the new right-align and border beats (thick-box route and per-edge walk).

#### 4.3-r2 pastes — the ROUND-2 page (historical; the round-3 delta above wins)
...
#### 4.4 blocksel — "Assemble and format the summary" · M — **ROUND 3 (playtest r2, r427)**
**ROUND-3 DELTA (Wolf playtest round 2, dev/ROUND2_FEEDBACK.md §2 blocksel) — supersedes the round-2 lines below where they conflict:**
- Beat 1 now says the DATA only: **"Copy the segment and revenue data into the summary block — the figures only, not the feed's headers"** (the check already required it; the label didn't say so).
- **"Operating income" is SPELLED OUT** — board header, feed caption, beats, hints, ALTS. Column D widens to carry it.
- The base revenue feed **arrives LEFT-ALIGNED** so the right-align beat is real work (Wolf: "doesn't help to right align the data if it's already coming right aligned"). The two CUT feeds stay right-aligned on purpose — that dress travels with the move.
- **NEW beat 6 — EBITDA margin % per segment** (EBITDA ÷ revenue) in a fifth table column. The Ctrl+A region moat moves from column E to column F. End-state graded (tolerance 6e-4) so a filled formula, four typed formulas or four typed percentages all clear (§1.0(c)). This reverses the round-2 "margin build dropped" line — Wolf reopened it.
- **☆ RE-CUT per §1.0-R2(i).** AUTO-EARN VERDICT on the round-2 ☆ ("both feeds travelled by CUT"): **it auto-earned** — the cut beats' own `ok` already required the sources to empty, so any player doing the core the taught way collected it with no separate decision. The new ☆ is Wolf's own suggestion: **all three lifts grabbed with the block-select chord (Ctrl+Shift+arrow) instead of walking the rect with plain Shift+arrow**. Graded off the r427 `S.grabLog` selection-provenance telemetry (rect + chord flag, stamped at copy/cut). Arrowed, moused and retyped routes clear every core — ALT 2 is exactly that run.
- **Beat count: 7 core** (+ save). §1.1 DEVIATION declared in the drill source.
- **Par re-swept:** parKeys 25→32, par 26→34 (5-seed median, ~1.06 s/key).

#### 4.4-r2 blocksel — the ROUND-2 page (historical; the round-3 delta above wins)
...
#### 4.5 rowops — "Rebuild the schedule" · M — **ROUND 3 (playtest r2, r427 — ABSORBS colops, D17)**
**ROUND-3 DELTA (Wolf playtest round 2, dev/ROUND2_FEEDBACK.md §2 rowops) — supersedes the round-2 lines below where they conflict:**
- **§1.0-R2(h) ACTION-LOCATION CUES.** Two loud RED notes, seeded by `build()`: **"<<< Insert row above"** in the spare gutter column beside the row the insert must push down, and **"Insert [Qn] column here ⌄"** in the empty row above the column the new quarter takes. Board furniture — the checks read column A labels, the header row and cell CONTENT, never the cue cells, so deleting a cue or a structure op sliding one cannot move any `ok`.
- **BUG R2-B3 FIXED at the drill level.** `targets()` was computing the staged-quarter ring from build coordinates with the DRAFT delete pre-applied (`o.sc-1`) — correct for one moment only; the instant the player inserted the quarter column the staged block slid back right and the ring sat one column off the data to be copied. All six rings now resolve by CONTENT SCAN at render time (predecessor label · PLACEHOLDER · Total row · FY header · DRAFT header · the staged block's own caption), so they track row inserts, row deletes, column deletes and column inserts alike. Scan lives in a `_rings()` helper so `targets()` stays a bare `return [...]` for the C9 parser.
- **§1.0-R2(j) LANGUAGE.** Beat 2 reads "the Total **recalculates** over all six lines"; the guide's "the Total re-ties on its own" → "recalculates on its own". This page's own beat-2 line is rewritten below.
- **☆ RE-CUT per §1.0-R2(i).** AUTO-EARN VERDICT on the round-2 ☆ ("every structure op from a FULL row/column selection"): **it auto-earned on the taught route** — Ctrl+± is *guarded* to full row/column selections, so `structFullN === structN` was a tautology the engine enforced for any chord user; there was no mastery decision inside it (it failed only on a ribbon-from-partial run, i.e. the slow route, not a choice). The new ☆ is Wolf's candidate: **the whole structure-chord family end-to-end** — space chords for the selections (`S.spaceSelN`) and **Ctrl+± for BOTH inserts and BOTH deletes** (`S.structChordN`), with **zero** ribbon structure ops (`S.structRibbonN === 0`). VERIFIED: the Home-ribbon route (Alt H I/D R·C, which works from a partial selection) clears all six cores and forfeits the star — ALT 1 in dev/e2e-alt-paths.js is exactly that run.
- **Par re-swept, unchanged** (30/29): the round-3 items are board cues + telemetry, no new keystrokes.

#### 4.5-r2 rowops — the ROUND-2 page (historical; the round-3 delta above wins)
...
2. Format the pasted figures to match the schedule (§1.0(a): straight away — the number format is the one thing inheritance can't know; ok also requires the Total to RECALCULATE over all six lines — §1.0-R2(j), r427)
