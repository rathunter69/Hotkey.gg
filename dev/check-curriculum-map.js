/* CURRICULUM MAP GUARD (r454, dev/CURRICULUM_REBUILD.md Phase A; rules re-cut to Wolf's
   ...
   --v5 (r458, dev/CURRICULUM_REBUILD.md Phase A): the SAME shape of proof, over
   dev/curriculum-v5.json — the v5.1 map (61 drills, 8 chapters, generated by
   dev/gen/curriculum-v5-json.py from curriculum_v51_data.py + curriculum_v51_map.py; the
   JSON is never hand-edited). Reuses the shared vocabulary and (a) require-before-teach
   proof above, then asserts, in catalog order: (2) every key appears exactly once, every
   chapter has exactly one capstone and it is last, c1-c4 are free and c5-c8 unlock at
   13/16/19/22; (3) every retired[] entry's `into` names a key the map carries; (4) the
   §4-style tag coverage report — a tag the vocabulary carries that nothing teaches fails,
   one nothing ever requires is printed as a warning only; (5) the difficulty spine (the v3
   x0.63 floor), skipping kind 'tutorial' and 'capstone', printed per chapter; (6) every
   drill with a non-empty engine[] is named in some dev/ENGINE_GAP_MATRIX.md pack's
   unblocks, printed as a pack -> drills-waiting table; (7) the headline counts (61 drills,
   8 chapters, 8 capstones, 36 retired), failing if they drift from what drills.js'
   menuOrder implies (live keys = kept + retired).

   Run: node dev/check-curriculum-map.js         (v3 — the gated one, exit 0 = clean)
        node dev/check-curriculum-map.js --v4    (v4 — the wave-0 map)
        node dev/check-curriculum-map.js --v5    (v5 — the v5.1 map) */
'use strict';
const fs = require('fs');
...
const MAP_PATH = path.join(ROOT, 'dev', V5 ? 'curriculum-v5.json' : V4 ? 'curriculum-v4.json' : 'curriculum-v3.json');
const DROP = 0.63;
...
if (V5) {
/* ---------- (2) membership: unique keys, one capstone last, tier + unlock curve ---------- */
{
  let bad2 = 0;
  const mapKeys = flat.map(f => f.d.key);
  const dupes = mapKeys.filter((k, i) => mapKeys.indexOf(k) !== i);
  if (dupes.length) { bad(`(2) key(s) listed more than once in the map: ${[...new Set(dupes)].join(', ')}`); bad2++; }

  for (const c of chapters) {
    const ds = c.drills || [];
    const caps = ds.filter(d => d.kind === 'capstone');
    if (caps.length !== 1) { bad(`(2) ${c.id}: ${caps.length} entries of kind 'capstone' (want exactly 1)`); bad2++; }
    const last = ds[ds.length - 1];
    if (!last || last.key !== c.capstone || last.kind !== 'capstone')
      { bad(`(2) ${c.id}: last entry is '${last && last.key}', not the declared capstone '${c.capstone}'`); bad2++; }
  }

  const FREE = ['c1', 'c2', 'c3', 'c4'], UNLOCK = { c5: 13, c6: 16, c7: 19, c8: 22 };
  for (const c of chapters) {
    if (FREE.includes(c.id)) {
      if (c.tier !== 'free' || c.unlock_level !== null)
        { bad(`(2) ${c.id} must be tier free / unlock_level null (has ${c.tier}/${c.unlock_level})`); bad2++; }
    } else if (UNLOCK[c.id] !== undefined) {
      if (c.tier !== 'pro' || c.unlock_level !== UNLOCK[c.id])
        { bad(`(2) ${c.id} must be tier pro / unlock_level ${UNLOCK[c.id]} (has ${c.tier}/${c.unlock_level})`); bad2++; }
    } else { bad(`(2) unexpected chapter id '${c.id}' — v5 expects exactly c1..c8`); bad2++; }
  }
  if (!bad2) ok(`(2) membership: ${mapKeys.length} unique keys · every chapter ends on its one capstone · c1-c4 free, c5-c8 unlock 13/16/19/22`);
}

/* ---------- (3) retired[] names an `into` key the map carries ---------- */
{
  let bad3 = 0;
  const mapKeySet = new Set(flat.map(f => f.d.key));
  const retired = M.retired || [];
  for (const r of retired)
    if (!r.into || !mapKeySet.has(r.into)) { bad(`(3) retired '${r.key}' names into '${r.into}', which is not a key in the map`); bad3++; }
  if (!bad3) ok(`(3) retired[]: ${retired.length} entries, every 'into' names a live map key`);
}

/* ---------- (4) tag vocabulary coverage ---------- */
{
  const vocab = M.tags || [];
  const taught = new Set(), required = new Set();
  for (const { d } of flat) {
    for (const t of (d.teaches || [])) taught.add(t);
    for (const t of (d.requires || [])) required.add(t);
  }
  const neverTaught = vocab.filter(t => !taught.has(t));
  const neverRequired = vocab.filter(t => !required.has(t));
  if (neverTaught.length) bad(`(4) tag(s) in the vocabulary that nothing teaches: ${neverTaught.join(', ')}`);
  else ok(`(4) vocabulary: all ${vocab.length} tags are taught by some drill`);
  if (neverRequired.length) console.warn(`WARN (4) ${neverRequired.length} tag(s) never required by anything: ${neverRequired.join(', ')}`);
}

/* ---------- (5) the difficulty spine (skip tutorial + capstone) ---------- */
{
  let bad5 = 0;
  console.log('\n  THE SPINE (par by position; (n) = tutorial/capstone — exempt from the floor)');
  for (const c of chapters) {
    const line = c.drills.map(d => (d.kind === 'tutorial' || d.kind === 'capstone') ? `(${d.par})` : d.par).join(' → ');
    console.log(`    ${c.id} ${c.name.padEnd(18)} ${line}`);
    const graded = c.drills.filter(d => d.kind !== 'tutorial' && d.kind !== 'capstone');
    for (let i = 0; i + 1 < graded.length; i++) {
      const a = graded[i], b = graded[i + 1];
      const r = b.par / a.par;
      if (r < DROP) { bad(`(5) ${c.id}: ${a.key} ${a.par} → ${b.key} ${b.par} is x${r.toFixed(2)} — below the x${DROP} floor`); bad5++; }
    }
  }
  if (!bad5) ok(`\n(5) spine monotone within tolerance in all ${chapters.length} chapters (floor x${DROP})`);
}

/* ---------- (6) engine_packs: every engine-gated drill is named in some pack's unblocks ---------- */
{
  let bad6 = 0;
  const packs = M.engine_packs || [];
  const mapKeySet = new Set(flat.map(f => f.d.key));
  const unblocked = new Set();
  for (const p of packs)
    for (const k of (p.unblocks || [])) {
      if (!mapKeySet.has(k)) { bad(`(6) pack ${p.id} unblocks '${k}', which is not a key in the map`); bad6++; }
      unblocked.add(k);
    }
  for (const { d } of flat)
    if ((d.engine || []).length && !unblocked.has(d.key))
      { bad(`(6) ${d.key} has a non-empty engine[] but is not named in any pack's unblocks`); bad6++; }
  console.log('\n  ENGINE PACKS (dev/ENGINE_GAP_MATRIX.md) — pack → drills waiting');
  for (const p of packs) console.log(`    ${p.id.padEnd(4)} ${p.name.padEnd(30)} ${(p.unblocks || []).join(', ')}`);
  if (!bad6) ok(`(6) engine packs: ${packs.length} packs, every engine-gated drill named in a pack's unblocks`);
}

/* ---------- (7) counts, checked against drills.js ---------- */
{
  let bad7 = 0;
  const nDrills = flat.length, nChapters = chapters.length;
  const nCapstones = flat.filter(f => f.d.kind === 'capstone').length;
  const nRetired = (M.retired || []).length;
  console.log(`\n  COUNTS: ${nDrills} drills · ${nChapters} chapters · ${nCapstones} capstones · ${nRetired} retired`);
  if (nDrills !== 61) { bad(`(7) ${nDrills} drills (want 61)`); bad7++; }
  if (nChapters !== 8) { bad(`(7) ${nChapters} chapters (want 8)`); bad7++; }
  if (nCapstones !== 8) { bad(`(7) ${nCapstones} capstones (want 8)`); bad7++; }
  if (nRetired !== 36) { bad(`(7) ${nRetired} retired (want 36)`); bad7++; }
  ...
  const live = D.menuOrder || (D.groups || []).flatMap(g => g.keys || []);
  const liveSet = new Set(live);
  const kept = flat.filter(f => liveSet.has(f.d.key)).length;
  if (live.length !== kept + nRetired)
    { bad(`(7) drills.js menuOrder has ${live.length} keys, but kept(${kept}) + retired(${nRetired}) = ${kept + nRetired}`); bad7++; }
  else ok(`(7) counts hold against drills.js: menuOrder ${live.length} keys = kept ${kept} + retired ${nRetired}`);
}
}

console.log(fail ? `\nCURRICULUM MAP${V5 ? ' (v5)' : ''}: ${fail} FAILURE(S)` : `\nCURRICULUM MAP${V5 ? ' (v5)' : ''}: clean`);
