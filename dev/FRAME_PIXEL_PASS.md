...
## STATUS — implemented r452 (worktree report, merged verbatim)
...
## 5 · STATUS — r452: SHIPPED across all 31 skins (2026-09-03)

Wolf: *"I love the pixel art card mock ups — implement fully across the board."* The three
decisions came back **D1 = 3px at `.hk-frame-lg`, 2px compact · D2 = cut particle blur/glow,
keep the card's ambient shadow · D3 = PRO and Founder go pixel too, finer grid + denser field.**
All three are implemented as decided.

### What landed

| slice | where | note |
|---|---|---|
| Pixel mode in the fx engine | `themes.js` `hkInitCardFx` | ONE engine, not two — see below |
| `px` in the skin's fx config | `themes.js` `SKINS` slot 6 | `pro` / `founder` declare `{px:2}`; everything else defaults to 3 |
| 30 skin blocks converted | `nav.css` | stepped borders · 3-band interiors · rim + no glow |
| Shared pixel rule set | `nav.css` | squared silhouette, square chips, pixelated canvas, hard notch shadow, mono names |
| Motion quantized | `nav.css` | `steps()` on all 10 keyframe users, **every period unchanged** |
| 11 dead fx branches deleted | `themes.js` | −155 lines |

### The engine: how one flag converts 25 draw branches

The naive reading of R6 — hand the seed functions the CELL dimensions — silently breaks three
things this pass must preserve: particle **counts** collapse (they are computed from `w*h`),
absolute **radii** become cell counts (bokeh's `r=28` would grow 6× and swallow the card — the
prototype's own failure mode), and **velocities** in px/frame triple the fall speed, changing
every period the spec freezes.

So the seeders and all 25 branches keep their device-space geometry **untouched**, and the
scale lives in a context wrapper: `pixelCtx(raw, s)` multiplies each incoming coordinate by
`s` (cells per device px) and *then* rounds, so the rounding happens in buffer space where it
belongs. On top of that it converts the soft primitives:

| primitive | becomes | rule |
|---|---|---|
| `arc()` / `ellipse()` + `fill()` | an integer square sprite, min 1 cell | R7 |
| `shadowBlur` | forced to 0, always | R4 / D2 |
| gradient colour stops | resampled to 6 hard bands, **endpoints exact** | R3 / R8 |
| `stroke()` | 1 cell wide, coords on `.5` centres | R2 |
| `rotate()` | snapped to 90° = a 4-frame sprite cycle | R7 |
| `font` | rescaled into cell space | R6 |
| rgba alpha | 8-step ladder with a floor | R7 (see note) |

Counts, sizes, speeds and periods are therefore **identical to today**; only the rendering
resolution and edge hardness change. The non-pixel path is byte-for-byte the old one.

**Two render-loop lessons, both found by looking at the PNGs:**

1. *Holding each gradient stop forward* (the obvious way to harden a ramp) destroys the ramp's
   SHAPE. The plaque/engraved `sheen` is a `transparent → bright → transparent` bar; holding
   the bright middle stop to the end turned a narrow highlight into a hard wash over half the
   card. Fixed by **resampling** the ramp at band centres instead.
2. Resampling then washed the card a second way: a canvas gradient **clamps to its terminal
   colours outside the axis**, so tinting band 0 spilled that tint across everything before the
   axis. Fixed by sampling the END bands at the exact endpoints and quantizing only the interior.

**ALPHA NOTE (a deliberate deviation).** R7 asks for 3 alpha levels on *particles*. A generic
wrapper cannot tell a particle's alpha from an ambient wash's, and snapping a `.06` wash to
`.33` is the "soft clouds swallowing the stats" failure in reverse. The ladder is therefore 8
steps with a floor: visibly quantized, but it does not blow out the 25 ambient fields. At 1–2
cells a particle's alpha level is carried by the sprite anyway.

### The stair corner: NOT shipped, and why

§3.5 called this the one real constraint, and it held. `clip-path` on the card eats the notch,
which rides OUTSIDE the card box at `top:-15px`. The prototype's fix (a) moves the notch onto
the host — but the host is `.pc-card` / `.panel.me`, rendered by **lb.js and nav.js**, outside
this change's lane. So the silhouette here is `border-radius:0` **without** the 2-step 45°
stair. The notch taxonomy at nav.css:735–756 is untouched and C3 passes. The stair stays
available to a later pass that can move the notch; option (b) (four corner pseudo-elements)
remains blocked because most skins already spend `::before`/`::after`.

### Still open

- The stair corner above.
- The plaque/engraved `sheen` is quantized and legible, and its diagonal edge now shows a
  proper chunky pixel staircase — but it is still a *broad* highlight rather than the spec's
  "3-cell hard column". Narrowing it means special-casing one branch, which would start the
  engine fork this pass exists to avoid. Left as-is deliberately.
- §3's proposed new C-check (every SKINS fx kind resolves to a live branch) is **not** added;
  the 11 dead kinds it would have caught are gone, so it now guards an empty set. Worth adding
  when the next fx kind lands.
