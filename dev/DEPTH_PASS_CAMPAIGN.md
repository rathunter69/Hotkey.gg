...
| Models II | 0 | 10 | ⚠️ read §5 below before dispatching — wave 3 (`schedule` `intsched` `lbo` `revolver` `waterfall`) next |
| Full Builds | 0 | 10 | ⚠️ read §5 below before dispatching |

**54 of 74 passed.** Catalog **74** (was 82; `hunt` closed it out at r439) — counted from `menuOrder`
