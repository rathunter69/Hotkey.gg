...
(function(){
    ...
    try{
      ...
      for(let i=localStorage.length-1;i>=0;i--){ const k=localStorage.key(i);
        if(k && (k.indexOf('hk_ghost_')===0 || k.indexOf('hk_cert_')===0 ||
                 k.indexOf('hk_level_seen_')===0 || k.indexOf('hk_tip_')===0)) localStorage.removeItem(k); }   // per-drill ghost replays + cert-offer latches (r417 audit: a surviving hk_cert_<track> latch blocked the next account's cert offers) · r455: hk_level_seen_<key> (the next account would meet level 1 with its coaching already silenced) and hk_tip_<name> (its three one-time tips already spent) — both are TEACHING state that belongs to the player, not to the machine, and dev/check-invariants.js C26 asserts they are in this sweep
    }catch(e){}
  ...
  function renderProfile(m, d){
    ...
    m.innerHTML = '<div class="pc-card pc-owner'+(__isFrame?' pc-bare':'')+'">' +
      ...
      '<div class="pc-scroll">' + __pcCard + '</div>' +
      '</div>';
    const c = $('pcClose'); if(c) c.onclick = closeProfile;
